...
if ( !class_exists( "WCWH_FoodBoardChart" ) ) 
{

class WCWH_FoodBoardChart extends WCWH_CRUD_Controller
{	
	public $Notices;
	public $className = "FoodBoardChart";
		...
		try
        {
        	...
        	switch ( $action )
        	{
				case "export":
					...
					{
						case 'summary':
							$datas['filename'] = 'Foodboard Summary ';
						...
						break;
						case 'item':
							$datas['filename'] = 'Foodboard Items';
							if( !empty( $datas['from_date'] ) ) $params['from_date'] = date( 'Y-m-d H:i:s', strtotime( $datas['from_date'] ) );
							if( !empty( $datas['to_date'] ) ) $params['to_date'] = date( 'Y-m-d H:i:s', strtotime( $datas['to_date']." 23:59:59" ) );
						break;
					}
	...
	protected function export_data_handler( $params = array() )
	{
		...
		{
			case 'summary':
				return $this->get_foodboard_summary( $params, [], [] );
			break;
			case 'category':
				return $this->get_foodboard_category( $params, [], [] );
			break;
			case 'item':
				return $this->get_foodboard_item( $params, [], [] );
			break;
	...
	public function export_form( $type = 'summary' )
	{
		...
		{
			case 'summary':
				do_action( 'wcwh_templating', 'chart/export-fb_summary-chart.php', $this->tplName['export'], $args );
			break;
			case 'category':
				do_action( 'wcwh_templating', 'chart/export-fb_category-chart.php', $this->tplName['export'], $args );
			break;
			case 'item':
				do_action( 'wcwh_templating', 'chart/export-fb_item-chart.php', $this->tplName['export'], $args );
			break;
		}
	}
	...
	{	
		...
		if( ! $this->datas )
		{
			switch ( strtolower( $display_type ) ) {
				case 'summary':
					$this->datas = $this->get_foodboard_summary( $params, [], [] );
					break;
				case 'category':
					$this->datas = $this->get_foodboard_category( $params, [], [] );
					break;
				case 'item':
					$this->datas = $this->get_foodboard_item( $params, [], [] );
					break;
			}
		}
		$datas = $this->datas;
		...
		switch ( strtolower( $display_type ) ) 
		{
			case 'summary':
				$x = 'months';
				$y = '~chart_group';
				$chart_args = [ 'colors_num' => 3, 'y_axis_title' => 'Amount', 'x_axis_title' => 'Date', 'unit' => 'RM', 'title' => 'FoodBoard Summary', 'interaction_mode' => 'point', 'tooltip' => 'currency' ];
				break;
			case 'category':
				$x = '~chart_group';
				$y = 'category_code-category';
				$chart_args = [ 'x_axis_title' => 'Amount(RM)', 'title' => 'FoodBoard Category', 'horizontal_bar' => true, 'labels_sort' => false, 'interaction_mode' => 'y', 'tooltip' => 'currency' ];
				break;
			case 'item':
				$x = '~chart_group';
				$y = 'item_code-item_name';
				$chart_args = [ 'x_axis_title' => 'Amount(RM)', 'title' => 'FoodBoard Item', 'horizontal_bar' => true, 'labels_sort' => false, 'interaction_mode' => 'y', 'tooltip' => 'currency' ];
				break;
		}
	...
	public function foodboard_summary( $filters = array(), $order = array() )
	{
		$action_id = 'foodboard_summary';
		$token = apply_filters( 'wcwh_generate_token', $action_id );
		...
		>
			...
			<?php
				if( ! $init )
				{
					$datas = $this->get_foodboard_summary( $this->filters, $order, [] );
	...
	public function foodboard_category( $filters = array(), $order = array() )
	{
		$action_id = 'foodboard_category';
		$token = apply_filters( 'wcwh_generate_token', $action_id );
		...
		>
			...
			<?php
				if( ! $init )
				{
					$datas = $this->get_foodboard_category( $this->filters, $order, [] );
	...
	public function foodboard_item( $filters = array(), $order = array() )
	{
		$action_id = 'foodboard_item';
		$token = apply_filters( 'wcwh_generate_token', $action_id );
		...
		>
			...
			<?php
				if( ! $init )
				{
					$datas = $this->get_foodboard_item( $this->filters, $order, [] );
	 ...
	 *	---------------------------------------------------------------------------------------------------
	 */
	public function get_foodboard_summary( $filters = [], $order = [], $args = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();
	...
	public function get_foodboard_category( $filters = [], $order = [], $args = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();
	...
	public function get_foodboard_item( $filters = [], $order = [], $args = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();
		...
		public function get_pos( $filters = [], $run = false )
		{
			...
			$field.= ", k.meta_value AS qty, ROUND( k.meta_value * n.meta_value, 3 ) AS weight, ROUND( l.meta_value, 2 ) AS price, ROUND( m.meta_value, 2 ) AS amount ";

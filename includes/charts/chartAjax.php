...
if ( !class_exists( "WCWH_Chart_AJAX" ) )
{
	
class WCWH_Chart_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Chart_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_charts_listing' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Chart_Controller' ) ) include_once( WCWH_DIR . "/includes/charts/chartCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_charts_submission' ) return false;
        ...
        $this->includes();
        $Inst = new WCWH_Chart_Controller();
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_charts_listing()
    {
        ...
        $outcome['datas']['chart'] = $Inst->get_chart_data( $form );

...
if ( !class_exists( "WCWH_EstateChart_AJAX" ) )
{
	
class WCWH_EstateChart_AJAX extends WCWH_AJAX
{
	protected $refs;
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'estate_summary' => false,
            'estate_category' => false,
            'estate_item' => false,
        ] );
    }
    ...
    {
        if( ! class_exists( 'WCWH_EstateChart' ) ) include_once( WCWH_DIR . "/includes/charts/EstateChart.php" ); 
    }

    public function submission()
    {
        $section_id = 'estate_export';
        $datas = $_REQUEST;
    ...
    public function estate_summary()
    {
        ...
        $outcome['segments']['#exportEstateChartTPL'] = $this->renew_form( $Inst, 'export_form', 'summary' );
    ...
    public function estate_category()
    {
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportEstateChartTPL'] = $this->renew_form( $Inst, 'export_form', 'category' );
        $outcome['datas']['chart'] = $Inst->get_chart( $form );
    ...
    public function estate_item()
    {
        $section_id = 'estate_item';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportEstateChartTPL'] = $this->renew_form( $Inst, 'export_form', 'item' );
        $outcome['datas']['chart'] = $Inst->get_chart( $form );
...
new WCWH_EstateChart_AJAX( $refs );
}
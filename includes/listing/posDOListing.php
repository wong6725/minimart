...
class WCWH_PosDO_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	public function column_ref_order( $item )
	{
		if( $item['ref_order_id'] )
		{
			$html = $item['ref_order_no'];
			
			$args = [ 'id'=>$item['ref_order_id'], 'service'=>'wh_pos_order_action', 'title'=>$html, 'permission'=>[ 'access_wh_good_issue' ] ];
			$html = $this->get_external_btn( $html, $args );
		}
		return $html;
	}

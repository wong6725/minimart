...
class WCWH_Adjustment_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	?>
		<div class="row">
			<div class="col-md-4">
				<label class="" for="flag">By Adjustment</label>
				<?php
					$options = [ ''=>'All', '+'=>"In +", '-'=>"Out -" ];
	                wcwh_form_field( 'filter[plus_sign]', 
	                    [ 'id'=>'', 'type'=>'select', 'label'=>'', 'required'=>false, 'attrs'=>[], 'class'=>['select2'],
	                        'options'=> $options, 
	                    ], 
	                    isset( $this->filters['plus_sign'] )? $this->filters['plus_sign'] : '', $view 
	                ); 
				?>
			</div>

...
class WCWH_IssueReturn_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	protected $section_id = "wh_issue_return";

	public $useFlag;

	protected $users;

	public $issue_returns;
	...
	?>
		<div class="row">
			...
			<div class="col-md-4">
				<label class="" for="flag">By Block Action</label>
				<?php
					$options = options_data( $this->issue_returns );
	                wcwh_form_field( 'filter[issue_return_type]', 
	                    [ 'id'=>'', 'type'=>'select', 'label'=>'', 'required'=>false, 'attrs'=>[], 'class'=>['select2'],
	                        'options'=> $options, 
	                    ], 
	                    isset( $this->filters['issue_return_type'] )? $this->filters['issue_return_type'] : '', $view 
	                ); 
				?>
			</div>

			<div class="col-md-4">
				<label class="" for="flag">By Action Reason</label>
				<?php
					$br = $this->block_reasons;
					foreach( $br as $b => $rs )
					{
						foreach( $rs as $k => $r ) $this->brs[$k] = $r;
					}
					$options = options_data( $this->brs );
	                wcwh_form_field( 'filter[block_reason]', 
	                    [ 'id'=>'', 'type'=>'select', 'label'=>'', 'required'=>false, 'attrs'=>[], 'class'=>['select2'],
	                        'options'=> $options, 
	                    ], 
	                    isset( $this->filters['block_reason'] )? $this->filters['block_reason'] : '', $view 
	                ); 
				?>
			</div>
		</div>
	...
	public function column_ref_doc( $item )
	{
		...
		if( $item['ref_doc_id'] && $item['ref_doc_type'] )
		{
			...
			{
				case 'block_stock':
					$args = [ 'id'=>$item['ref_doc_id'], 'service'=>'wh_block_stock_action', 'title'=>$html, 'permission'=>[ 'access_wh_block_stock' ] ];
				break;
			}
	...
	public function column_issue_return( $item )
	{
		return $this->issue_returns[ $item['issue_return_type'] ];
	}

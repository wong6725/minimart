...
class WCWH_OrderingPR_Listing  extends WCWH_Listing 
{	
	protected $refs;
	...
	public function get_bulk_actions() 
	{
		...
		if( current_user_can( 'save_'.$this->section_id ) )
			$actions['save'] = 'Create Purchase Request';
	...
	?>
		<div class="row">
			...
			<div class="col-md-4">
				...
				<?php
					$wh_filter = ['status'=>1, 'visible'=>1];
	                if($this->warehouse['indication'] && !$this->warehouse['view_outlet']) $wh_filter['not_id'] = $this->warehouse['id'];
	                $wh_options = options_data( apply_filters( 'wcwh_get_warehouse', $wh_filter, [], false, [] ), 'code', [ 'code', 'name' ] );

	                wcwh_form_field( $prefixName.'filter[warehouse_id]', 
	...
	public function column_warehouse_id( $item ) 
	{

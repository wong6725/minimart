...
class WCWH_Warehouse_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	public function column_capability( $item )
	{
		if( $item['capability'] && is_json( $item['capability'] ) )
			$item['capability'] = json_decode( $item['capability'], true );

		if( $this->wh_capabilities )
			$options = options_data( $this->wh_capabilities, 'key', [ 'title' ] );

		$cap = array();
		if( $item['capability'] )
		foreach( $item['capability'] as $capability )
		{
	...
	public function column_indication( $item )
	{
		return ( $item['indication'] )? 'This' : ' - ';
	}

...
class WCWH_Asset_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	public function column_serial( $item )
	{
		...
		if( $item['serial'] ) 
		{	
			$datas = [ 'dataitem'=>$item['name'], 'dataserial'=>$item['serial'] ];
			
			$actions['qr'] = $this->get_action_btn( $item, 'qr', [ 'serial'=>$item['serial'], 'tpl'=>'asset_label', 'datas'=>$datas ] );
			$actions['barcode'] = $this->get_action_btn( $item, 'barcode', [ 'serial'=>$item['serial'], 'tpl'=>'asset_label', 'datas'=>$datas ] );
		}

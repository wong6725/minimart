...
class WCWH_GoodReturn_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	public function action_btn_addon( $btn, $item, $action = "view", $args = array() )
	{
		...
		{
			case 'return_note':
				$btn = '<a class="btn btn-xs btn-info toggle-modal" data-action="print" data-tpl="printRTN" 
				 	data-title="Print Return Note" data-modal="wcwhModalImEx" data-actions="close|printing" 
					data-id="'.$id.'"
					><i class="fa fa-print" aria-hidden="true"></i> RN</a>';
			break;
		}
	...
	public function column_dn( $item )
	{
		$actions = [];
		//$actions[] = $this->get_action_btn( $item, 'print', [ 'print_type'=>'delivery_note', 'doc_code'=>'DN', 'title_addon'=>'Delivery Note' ] );
		//$actions[] = $this->get_action_btn( $item, 'print', [ 'print_type'=>'delivery_note', 'doc_code'=>'DN c', 'title_addon'=>'Delivery Note by Category', 'view_type'=>'category' ] );
		if( $item['status'] >= 6 )
			$actions['dn'] = $this->get_action_btn( $item, 'return_note' ); 
	...
	public function column_gr( $item )
	{	
		$html = $item['ref_doc'];
		if( $item['ref_doc_id'] && $item['ref_doc_type'] == 'good_receive' )
		{

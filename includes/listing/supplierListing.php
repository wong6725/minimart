...
class WCWH_Supplier_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	public function get_columns() 
	{
		return array(
			...
			"name" 		=> "Supplier",
			"code" 		=> "Code",
			"supplier_no" => "SAP Supplier No.",
			//"comp_id"	=> "Company",
			"self_bill"	=> "Self Bill EInv",
			"parent" 	=> "Parent",
			"status"	=> "Status",
	...
	{
		$col = array( "created_by", "created_at", "lupdate_by", "lupdate_at" );
	...
	public function column_self_bill( $item )
	{
		$html = !empty( $item['self_bill'] )? 'YES' : 'NO';
		if( !empty( $item['tin'] ) ) $html.= "<br>{$item['tin']}";

...
class WCWH_SaleReturn_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	public function get_columns() 
	{
		return array(
			...
			"docno" 		=> "Doc No.",
			"ref_doc"		=> "Ref Doc",
			"return_action"	=> "Return Type",
			"return_reason"	=> "Action Reason",
			"doc_date"		=> "Doc Date",
			"post_date"		=> "Post Date",
			"status"		=> "Status",
	...
	?>
		<div class="row">
			...
			<div class="col-md-4">
				<label class="" for="flag">By Return Type</label>
				<?php
					$options = options_data( $this->return_actions );
	                wcwh_form_field( 'filter[return_type]', 
	                    [ 'id'=>'', 'type'=>'select', 'label'=>'', 'required'=>false, 'attrs'=>[], 'class'=>['select2'],
	                        'options'=> $options, 
	                    ], 
	                    isset( $this->filters['return_type'] )? $this->filters['return_type'] : '', $view 
	                ); 
				?>
			</div>

			<div class="col-md-4">
				<label class="" for="flag">By Return Reason</label>
				<?php
					$br = $this->return_reasons;
					foreach( $br as $b => $rs )
					{
						foreach( $rs as $k => $r ) $this->brs[$k] = $r;
					}
					$options = options_data( $this->brs );
	                wcwh_form_field( 'filter[return_reason]', 
	                    [ 'id'=>'', 'type'=>'select', 'label'=>'', 'required'=>false, 'attrs'=>[], 'class'=>['select2'],
	                        'options'=> $options, 
	                    ], 
	                    isset( $this->filters['return_reason'] )? $this->filters['return_reason'] : '', $view 
	                ); 
				?>
			</div>
		</div>
	...
	public function column_ref_doc( $item )
	{
		...
		if( $item['ref_doc_id'] && $item['ref_doc_type'] )
		{
			switch( $item['ref_doc_type'] )
			{
				case 'good_return':
					$args = [ 'id'=>$item['ref_doc_id'], 'service'=>'wh_good_return_action', 'title'=>$html, 'permission'=>[ 'access_wh_good_return' ] ];
				break;
			}
	...
	public function column_return_action( $item )
	{
		return $this->return_actions[ $item['return_type'] ];
	}

	public function column_return_reason( $item )
	{
		return $this->brs[ $item['return_reason'] ];
	}

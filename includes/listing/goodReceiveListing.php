...
class WCWH_GoodReceive_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	{	
		...
		if( $item['hid'] && $item['tr_status'] )
			unset( $actions['6']['unpost'] );
	...
	?>
		<div class="row">
			...
			<div class="col-md-4">
				<label class="" for="flag">By Reference Document</label>
				<?php
					$options = [ ''=>'All', 'purchase_order'=> 'Purchase Order','delivery_order'=>'Delivery Order', 'block_action'=>'Block Stock Action' ];
	                wcwh_form_field( 'filter[ref_doc_type]', 
	                    [ 'id'=>'', 'type'=>'select', 'label'=>'', 'required'=>false, 'attrs'=>[], 'class'=>['select2'],
	                        'options'=> $options, 
	                    ], 
	                    isset( $this->filters['ref_doc_type'] )? $this->filters['ref_doc_type'] : '', $view 
	                ); 
				?>
			</div>
		</div>
	...
	{	
		...
		else if( $item['ref_doc_id'] && $item['ref_doc_type'] == 'block_action' )
		{
			$html = $item['ref_doc'];
			$args = [ 'id'=>$item['ref_doc_id'], 'service'=>'wh_block_action_action', 'title'=>$html, 'permission'=>[ 'access_wh_block_action' ] ];
			$html = $this->get_external_btn( $html, $args );
		}
	...
	public function column_po( $item )
	{
		...
		if( $item['ref_doc_id'] && $item['ref_doc_type'] == 'purchase_order' )
		{

...
class WCWH_MoneyCollector_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	public function get_bulk_actions() 
	{
		...
		if( current_user_can( 'access_wh_money_collector' ) )
	...
	public function get_status_action( $item )
	{
		...
		return array(
			...
			'1' => array(
				'view' => [ 'wcwh_user' ],
				'edit' => [ 'access_wh_money_collector'],
				'delete' => [ 'access_wh_money_collector'],
				'post' => [ 'access_wh_money_collector'],
			...
			'6' => array(
				'view' => [ 'wcwh_user' ],
				'edit' => [ 'wh_admin_support' ],
				'unpost' => [ 'access_wh_money_collector'],
				'complete' => [ 'access_wh_money_collector'],
			),
			'9' => array(
				'view' => [ 'wcwh_user' ],
				// 'edit' => [ 'wh_admin_support' ],
				//'duplicate' => [ 'duplicate_'.$this->section_id ],
				'incomplete' => [ 'access_wh_money_collector' ],
	...
	public function action_btn_addon( $btn, $item, $action = "view", $args = array() )
	{
		...
		switch( $action )
		{
			case 'money_collector':
				$btn = '<a class="btn btn-xs btn-info toggle-modal" data-action="print" data-tpl="printMC" 
				 	data-title="Print Money Collector" data-modal="wcwhModalImEx" data-actions="close|printing" 
					data-id="'.$id.'"
					><i class="fa fa-print" aria-hidden="true"></i> MC</a>';
			break;
	...
	public function column_print( $item )
	{
			...
			if( $item['status'] > 0 )
			{
				$actions['money_collector'] = $this->get_action_btn( $item, 'money_collector',['force'=>1] ); 
			}

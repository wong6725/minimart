...
class WCWH_PosOrder_Listing extends WCWH_Listing 
{	
	protected $refs;
	...
	{	
		...
		if( ! in_array( $item['order_status'], [ 'wc-cancelled', 'wc-refunded'. 'wc-failed' ] ) )
			$actions['edit'] = $this->get_action_btn( $item, 'edit' );

		if( in_array( $item['order_status'], [ 'wc-processing', 'wc-completed'. 'wc-pending', 'wc-on-hold' ] ) )
	...
	{
		$html = [];
		if( $item['register'] ) $html[] = $item['register'];
		if( $item['ip'] ) $html[] = $item['ip'];

...
if ( !class_exists( "WCWH_Simplify_Reprocess_AJAX" ) )
{
	
class WCWH_Simplify_Reprocess_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Simplify_Reprocess_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_simplify_reprocess_action' => false,
            'wh_simplify_reprocess_listing' => false,
            'wh_simplify_reprocess_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Simplify_Reprocess_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/reprocessCtrlv2.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_simplify_reprocess_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'reprocess_reference' ) ) )
        {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#reprocess_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'reprocess_reference':
    ...
    public function wh_simplify_reprocess_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_simplify_reprocess_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_Simplify_Reprocess_AJAX( $refs );
}
...
if ( !class_exists( "WCWH_AccPeriod_AJAX" ) )
{
	
class WCWH_AccPeriod_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "AccPeriod_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_acc_period_action' => false,
            'wh_acc_period_listing' => false,
            'wh_acc_period_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_AccPeriod_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/accPeriodCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_acc_period_action()
    {
    ...
    public function wh_acc_period_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_acc_period_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

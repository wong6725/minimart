...
if ( !class_exists( "WCWH_RemoteCPR_AJAX" ) )
{
	
class WCWH_RemoteCPR_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "RemoteCPR_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_remote_cpr_action' => false,
            'wh_remote_cpr_listing' => false,
            'wh_remote_cpr_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_RemoteCPR_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/remoteCPRCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_remote_cpr_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'remote_cpr_reference' ) ) )
        {
    ...
    public function wh_remote_cpr_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_remote_cpr_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

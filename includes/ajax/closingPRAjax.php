...
if ( !class_exists( "WCWH_ClosingPR_AJAX" ) )
{
	
class WCWH_ClosingPR_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "ClosingPR_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_closing_pr_action' => false,
            'wh_closing_pr_listing' => false,
            'wh_closing_pr_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_ClosingPR_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/closingPRCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_closing_pr_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'closing_pr_reference' ) ) )
        {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#closing_pr_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'closing_pr_reference':
                        ob_start();
    ...
    public function wh_closing_pr_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_closing_pr_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

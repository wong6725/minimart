...
if ( !class_exists( "WCWH_TODO_AJAX" ) )
{
	
class WCWH_TODO_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "TODO_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_todo_action' => false,
            'wh_todo_listing' => false,
            'wh_todo_history_action' => false,
            'wh_todo_history_listing' => false,
        ] );
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_todo_action()
    {
    ...
    public function wh_todo_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_todo_history_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
                ...
                switch( $action )
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            //reload content
                            if( ! $datas['section'] || ( $datas['section'] && $datas['section'] == $section_id.'-listing-form' ) )
                                $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $filters, [], 'view_history_listing' );
                        }
                    break;
                }
            }
    ...
    public function wh_todo_history_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], 'view_history_listing' );

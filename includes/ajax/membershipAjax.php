...
if ( !class_exists( "WCWH_Membership_AJAX" ) )
{
    
class WCWH_Membership_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "Membership_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_membership_action' => false,
            'wh_membership_listing' => false,
            'wh_membership_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Membership_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/membershipCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_membership_submission' ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_membership_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
                ...
                switch( $action )
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            $outcome['segments']['.action-group #membership_reference_content'] = $this->renew_form( $Inst, 'view_reference' );
                        }

                        if( $result['modal'] && $result['modal_form'] )
                        {
                            if(  method_exists( $Inst, $result['modal_form'] ) )
                            {
                                $func = $result['modal_form'];
                                ob_start();
                                $Inst->$func( $result['modal_data'] );                            
                                $outcome['content']['.modal-body'] = ob_get_clean();
                                $outcome['modal'] = $result['modal'];
                            }
                        }
                    break;
    ...
    public function wh_membership_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_membership_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        if( $succ && $form )
        {
            ...
            if( $succ )
            {
                $form[$key]['attachment']=$form['_attachment'];

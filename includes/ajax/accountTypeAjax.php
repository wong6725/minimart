...
if ( !class_exists( "WCWH_AccountType_AJAX" ) )
{
	
class WCWH_AccountType_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "AccountType_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_account_type_action' => false,
            'wh_account_type_listing' => false,
            'wh_account_type_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_AccountType_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/accountTypeCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_account_type_action()
    {
    ...
    public function wh_account_type_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_account_type_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_AccountType_AJAX( $refs );
}
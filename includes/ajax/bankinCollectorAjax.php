...
if ( !class_exists( "WCWH_BankInCollector_AJAX" ) )
{
	
class WCWH_BankInCollector_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "BankInCollector_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_bankin_collector_action' => false,
            'wh_bankin_collector_listing' => false,
            'wh_bankin_collector_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_BankInCollector_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/bankinCollectorCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_bankin_collector_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_bankin_collector_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        if( $succ && $form )
        {
            if( $form['action'] )
            {
                ...
                switch( $action )
                {
                    ...
                    break;
                    case 'bankin_collector_reference':
                        ob_start();
                            $Inst->gen_form( $form['id'] );
                        $outcome['content']['.modal-body'] = ob_get_clean();
                    break;
                }
                $outcome['segments']['.template-container #'.$Inst->tplName['bic'].'TPL'] = $this->renew_form( $Inst, 'bic_form' );
            }
    ...
    public function wh_bankin_collector_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_bankin_collector_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_BankInCollector_AJAX( $refs );
}
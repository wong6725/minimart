...
if ( !class_exists( "WCWH_IssueReturn_AJAX" ) )
{
	
class WCWH_IssueReturn_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "IssueReturn_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_issue_return_action' => false,
            'wh_issue_return_listing' => false,
            'wh_issue_return_form' => false,
        ] );
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_issue_return_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'issue_return_reference' ) ) )
        {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#issue_return_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'issue_return_reference':
                        ob_start();
    ...
    public function wh_issue_return_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_issue_return_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_IssueReturn_AJAX( $refs );
}
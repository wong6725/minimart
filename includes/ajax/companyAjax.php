...
if ( !class_exists( "WCWH_Company_AJAX" ) )
{
	
class WCWH_Company_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Company_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_company_action' => false,
            'wh_company_listing' => false,
            'wh_company_form' => false,
        ] );
    }
     ...
     *	---------------------------------------------------------------------------------------------------
     */
    public function wh_company_action()
    {
    ...
    public function wh_company_listing()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
    	$outcome = array();
    ...
    public function wh_company_form()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
            ...
            if( $datas['section'] )
            {
                ...
                switch( $section )
                {
                    case 'wh_warehouse':
                        ...
                        $Wh = new WCWH_Warehouse_Controller();
                        $outcome['segments']['.template-container #'.$Wh->tplName['new'].'TPL'] = $this->renew_form( $Wh );
                    break;
                    case 'wh_customer':
                    ...
                    case 'wh_supplier':
                        ...
                        $outcome['segments']['.template-container #'.$Supplier->tplName['new'].'TPL'] = $this->renew_form( $Supplier );
                    break;
                    case 'wh_brand':
                        include_once( WCWH_DIR."/includes/controller/brandCtrl.php" ); 
                        $Brand = new WCWH_Brand_Controller();
                        $outcome['segments']['.template-container #'.$Brand->tplName['new'].'TPL'] = $this->renew_form( $Brand );
                    break;
                }
            }
        }

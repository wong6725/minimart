...
if ( !class_exists( "WCWH_Credit_AJAX" ) )
{
	
class WCWH_Credit_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Credit_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_credit_action' => false,
            'wh_credit_listing' => false,
            'wh_credit_form' => false,
        ] );
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_credit_action()
    {
                ...
                switch( $action )
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            $outcome['segments']['.template-container #'.$Inst->tplName['newGroup'].'TPL'] = $this->renew_form( $Inst, 'view_group_form' );
                            $outcome['segments']['.template-container #'.$Inst->tplName['newCustomer'].'TPL'] = $this->renew_form( $Inst, 'view_customer_form' );
                        }
                    break;
                }
            }
    ...
    public function wh_credit_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_credit_form()
    {   
        $this->Notices->reset_operation_notice();
        $succ = true;

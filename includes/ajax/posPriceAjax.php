...
if ( !class_exists( "WCWH_PosPrice_AJAX" ) )
{
	
class WCWH_PosPrice_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PosPrice_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
			'wh_pos_price_listing' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_POSPrice_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/posPriceCtrl.php" ); 
    }

    public function submission()
    {
		$section_id = 'pos_price_report';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_pos_price_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportPosPriceTPL'] = $this->renew_form( $Inst, 'export_form', '' );

<?php
//Steven create UOM Conversion AJAX based on UOM AJAX
if (!defined('ABSPATH'))
    exit; // Exit if accessed directly

if ( !class_exists( "WCWH_UOMConversion_AJAX" ) )
{
	
class WCWH_UOMConversion_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "UOMConversion_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_uom_conversion_action' => false,
            'wh_uom_conversion_listing' => false,
            'wh_uom_conversion_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_UOMConversion_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/uomConversionCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_uom_conversion_action()
    {
    ...
    public function wh_uom_conversion_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_uom_conversion_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_UOMConversion_AJAX( $refs );
}
...
if ( !class_exists( "WCWH_GoodReceive_AJAX" ) )
{
	
class WCWH_GoodReceive_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "GoodReceive_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_good_receive_action' => false,
            'wh_good_receive_listing' => false,
            'wh_good_receive_form' => false,
        ] );
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_good_receive_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'good_receive_reference' ) ) )
        {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#good_receive_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'good_receive_reference':
                        ob_start();
    ...
    public function wh_good_receive_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_good_receive_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_GoodReceive_AJAX( $refs );
}
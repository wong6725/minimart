...
if ( !class_exists( "WCWH_PosTransact_AJAX" ) )
{
	
class WCWH_PosTransact_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PosTransact_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_pos_transact_action' => false,
            'wh_pos_transact_listing' => false,
            'wh_pos_transact_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_PosTransact_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/posTransactCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_pos_transact_action()
    {
    ...
    public function wh_pos_transact_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_pos_transact_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_PosTransact_AJAX( $refs );
}
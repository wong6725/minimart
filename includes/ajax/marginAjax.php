...
if ( !class_exists( "WCWH_Margin_AJAX" ) )
{
	
class WCWH_Margin_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Margin_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_margin_action' => false,
            'wh_margin_listing' => false,
            'wh_margin_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_margin_submission' ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_margin_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        if( $succ && $form )
        {
            ...
            else if( $succ && $listForm && ( isset( $listForm["action"] ) || isset( $listForm["action2"] ) ) )
            {
                ...
                {
                    if( $listForm['action'] == 'wh_margin' )
                    {
                        $outcome['modal']['title'] = "New Margin";
    ...
    public function wh_margin_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_margin_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

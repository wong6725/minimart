...
if ( !class_exists( "WCWH_PaymentMethod_AJAX" ) )
{
	
class WCWH_PaymentMethod_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PaymentMethod_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_payment_method_action' => false,
            'wh_payment_method_listing' => false,
            'wh_payment_method_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_PaymentMethod_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/paymentMethodCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_payment_method_action()
    {
    ...
    public function wh_payment_method_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_payment_method_form()
    {   
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_PaymentMethod_AJAX( $refs );
}
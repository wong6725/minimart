...
if ( !class_exists( "WCWH_PurchaseRequest_AJAX" ) )
{
	
class WCWH_PurchaseRequest_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PurchaseRequest_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_purchase_request_action' => false,
            'wh_purchase_request_listing' => false,
            'wh_purchase_request_form' => false,
        ] );
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_purchase_request_action()
    {
        ...
        if( $succ && $form )
        {
            if( $form['action'] )
            {
                ...
                switch( $action )
                {
                    ...
                    break;
                    //----- Form Restoration 09/11/22
                    case 'purchase_request_tool_reference':
                        ob_start();
                            $Inst->gen_tool_form();
                        $outcome['content']['.modal-body'] = ob_get_clean();
                    break;
            ...
            else if( $succ && $listForm && ( isset( $listForm["action"] ) || isset( $listForm["action2"] ) ) )
            {
                ...
                else
                {
                    ...
                    else if ( in_array( $listForm['action'], ['duplicate'] ) )
                    {
                        switch( $listForm['action'] )
                        {
                            case 'duplicate':
                                ...
                                $outcome['content']['.modal-body'] = ob_get_clean();
                                $outcome['modal']['tpl'] = '';
                                $outcome['modal']['service'] = $section_id.'_action';
                                $outcome['modal']['actionBtn'] = [ 'close', 'submit' ];
    ...
    public function wh_purchase_request_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_purchase_request_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_PurchaseRequest_AJAX( $refs );
}
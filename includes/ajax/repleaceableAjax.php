...
if ( !class_exists( "WCWH_Repleaceable_AJAX" ) )
{
	
class WCWH_Repleaceable_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Repleaceable_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_repleaceable_action' => false,
            'wh_repleaceable_form' => false,
            'wh_repleaceable_listing' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Repleaceable_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/repleaceableCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_repleaceable_action()
    {
    ...
    public function wh_repleaceable_form()
    {
        $this->Notices->reset_operation_notice();
    	$succ = true;
    	...
    	if( $succ && $form )
    	{
    		$succ = $Inst->save_total($form['_repleaceable']);
            $this->Notices->set_notices( $Inst->Notices->get_operation_notice() );
    	}
        else
            $succ = false;
    ...
    public function wh_repleaceable_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
...
new WCWH_Repleaceable_AJAX( $refs );
}
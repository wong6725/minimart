...
if ( !class_exists( "WCWH_TransferItem_AJAX" ) )
{
	
class WCWH_TransferItem_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "TransferItem_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_transfer_item_action' => false,
            'wh_transfer_item_listing' => false,
            'wh_transfer_item_form' => false,
            'wh_transfer_item_by_gr_listing' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_TransferItem_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/transferItemCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_transfer_item_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'transfer_item_reference' ) ) )
        {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#transfer_item_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'transfer_item_reference':
                        ob_start();
    ...
    public function wh_transfer_item_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_transfer_item_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_transfer_item_by_gr_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
...
new WCWH_TransferItem_AJAX( $refs );
}
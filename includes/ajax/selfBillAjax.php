...
if ( !class_exists( "WCWH_SelfBill_AJAX" ) )
{
	
class WCWH_SelfBill_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "SelfBill_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_self_bill_action' => false,
            'wh_self_bill_listing' => false,
            'wh_self_bill_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_SelfBill_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/selfBillCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'print', 'wcwh_wh_self_bill_submission' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_self_bill_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'self_bill_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#self_bill_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'self_bill_reference':
                        ob_start();
                            $a = $Inst->gen_form( $form['id'] );
                            if($a == false)
                            {
                                $succ = false;
                                $this->Notices->set_notices( $Inst->Notices->get_operation_notice() );
                                $outcome['content'] = ob_end_clean();
                            }
                            else
                            {
                                $outcome['content']['.modal-body'] = ob_get_clean();
    ...
    public function wh_self_bill_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_self_bill_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

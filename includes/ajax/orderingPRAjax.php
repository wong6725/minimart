...
if ( !class_exists( "WCWH_OrderingPR_AJAX" ) )
{
	
class WCWH_OrderingPR_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "OrderingPR_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_ordering_pr_action' => false,
            'wh_ordering_pr_listing' => false,
            'wh_ordering_pr_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_OrderingPR_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/orderingPRCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_ordering_pr_action()
    {
    ...
    public function wh_ordering_pr_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_ordering_pr_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

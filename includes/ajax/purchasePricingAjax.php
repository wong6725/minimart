...
if ( !class_exists( "WCWH_PurchasePricing_AJAX" ) )
{
	
class WCWH_PurchasePricing_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PurchasePricing_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_purchase_pricing_action' => false,
            'wh_purchase_pricing_listing' => false,
            'wh_purchase_pricing_form' => false,
            'wh_latest_purchase_pricing_listing' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_PurchasePricing_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/purchasePricingCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_purchase_pricing_submission' ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_purchase_pricing_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        if( $succ && $form )
        {
            ...
            else if( $succ && $listForm && ( isset( $listForm["action"] ) || isset( $listForm["action2"] ) ) )
            {
                ...
                {
                    if( $listForm['action'] == 'wh_purchase_pricing' )
                    {
                        $outcome['modal']['title'] = "New PurchasePricing";

                        ob_start();
                            $Inst->gen_form( $listForm['id'] );
                        $outcome['content']['.modal-body'] = ob_get_clean();
                    }
                    else
                    {
    ...
    public function wh_purchase_pricing_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_purchase_pricing_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        if( $succ && $form )
        {
            ...
            if( $succ )
            {
                ...
                {
                    $outcome['modal']['title'] = "New PurchasePricing";
                    $outcome['modal']['actionBtn'] = array( 'close', 'submit' );
                    $outcome['modal']['modal'] = "wcwhModalForm";

                    ob_start();
                        $Inst->gen_import_form();
                    $outcome['content']['#wcwhModalForm .modal-body'] = ob_get_clean();
                }*/
            }
        }
        else
    ...
    public function wh_latest_purchase_pricing_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-latest-listing-form'] = $this->renew_listing( $Inst, $form, [], 'latest_price_listing' );
        //$outcome['segments']['#exportPriceTPL'] = $this->renew_form( $Inst, 'export_form' );
...
new WCWH_PurchasePricing_AJAX( $refs );
}
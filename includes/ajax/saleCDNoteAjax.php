...
if ( !class_exists( "WCWH_SaleCDNote_AJAX" ) )
{
	
class WCWH_SaleCDNote_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "SaleCDNote_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_sale_cdnote_action' => false,
            'wh_sale_cdnote_listing' => false,
            'wh_sale_cdnote_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_sale_cdnote_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_sale_cdnote_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'sale_cdnote_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#sale_cdnote_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'sale_cdnote_reference':
                        ob_start();
    ...
    public function wh_sale_cdnote_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_sale_cdnote_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

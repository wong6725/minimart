...
if ( !class_exists( "WCWH_ItemCategory_AJAX" ) )
{
	
class WCWH_ItemCategory_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "ItemCategory_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_items_category_action' => false,
            'wh_items_category_listing' => false,
            'wh_items_category_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_ItemCategory_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/itemCategoryCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_items_category_action()
    {
    ...
    public function wh_items_category_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_items_category_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
            ...
            if( $datas['section'] )
            {
                ...
                switch( $section )
                {
                    case 'wh_items':
                        ...
                        $Items = new WCWH_Item_Controller();
                        $outcome['segments']['.template-container #'.$Items->tplName['new'].'TPL'] = $this->renew_form( $Items );
                    break;
                    case 'wh_asset':
                        include_once( WCWH_DIR."/includes/controller/assetCtrl.php" ); 
                        $Asset = new WCWH_Asset_Controller();
                        $outcome['segments']['.template-container #'.$Asset->tplName['new'].'TPL'] = $this->renew_form( $Asset );
                    break;
                }
            }
        }
...
new WCWH_ItemCategory_AJAX( $refs );
}
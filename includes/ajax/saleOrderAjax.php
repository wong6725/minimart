...
if ( !class_exists( "WCWH_SaleOrder_AJAX" ) )
{
	
class WCWH_SaleOrder_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "SaleOrder_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_sales_order_action' => false,
            'wh_sales_order_listing' => false,
            'wh_sales_order_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'print', 'wcwh_wh_sales_order_submission' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_sales_order_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'sale_order_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#sale_order_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'sale_order_reference':
                        ob_start();
    ...
    public function wh_sales_order_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_sales_order_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

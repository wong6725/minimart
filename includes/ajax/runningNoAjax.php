...
if ( !class_exists( "WCWH_RunningNo_AJAX" ) )
{
	
class WCWH_RunningNo_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "RunningNo_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_runningno_action' => false,
            'wh_runningno_listing' => false,
            'wh_runningno_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_RunningNo_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/RunningNoCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_runningno_action()
    {
    ...
    public function wh_runningno_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_runningno_form()
    {   
        $this->Notices->reset_operation_notice();
        $succ = true;

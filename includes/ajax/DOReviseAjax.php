...
if ( !class_exists( "WCWH_DORevise_AJAX" ) )
{
	
class WCWH_DORevise_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "DORevise_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_do_revise_action' => false,
            'wh_do_revise_listing' => false,
            'wh_do_revise_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_DORevise_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/DOReviseCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_do_revise_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_do_revise_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'do_revise_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#do_revise_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'do_revise_reference':
                        ob_start();
    ...
    public function wh_do_revise_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_do_revise_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

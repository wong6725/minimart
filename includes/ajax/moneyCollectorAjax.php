...
if ( !class_exists( "WCWH_MoneyCollector_AJAX" ) )
{
	
class WCWH_MoneyCollector_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "MoneyCollector_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_money_collector_action' => false,
            'wh_money_collector_listing' => false,
            'wh_money_collector_form' => false,
            'wh_money_collector_form_update' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_MoneyCollector_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/moneyCollectorCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'print', 'wcwh_wh_money_collector_submission' ] ) ) return false;
     ...
     *	---------------------------------------------------------------------------------------------------
     */
    public function wh_money_collector_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_money_collector_listing()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
    	$outcome = array();
    ...
    public function wh_money_collector_form()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
    ...
    // public function wh_money_collector_form_update()
    // {
    //     $this->Notices->reset_operation_notice();
    //     $succ = true;
...
new WCWH_MoneyCollector_AJAX( $refs );
}
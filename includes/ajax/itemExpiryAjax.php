...
if ( !class_exists( "WCWH_ItemExpiry_AJAX" ) )
{
	
class WCWH_ItemExpiry_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "ItemExpiry_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_item_expiry_action' => false,
            'wh_item_expiry_listing' => false,
            'wh_item_expiry_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_ItemExpiry_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/itemExpiryCtrl.php" ); 
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_item_expiry_action()
    {
                ...
                switch( $action )
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            $outcome['segments']['.template-container #'.$Inst->tplName['newCategory'].'TPL'] = $this->renew_form( $Inst, 'view_category_form' );
                            $outcome['segments']['.template-container #'.$Inst->tplName['newItem'].'TPL'] = $this->renew_form( $Inst, 'view_item_form' );
                        }
                    break;
                }
            }
    ...
    public function wh_item_expiry_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_item_expiry_form()
    {   
        $this->Notices->reset_operation_notice();
        $succ = true;

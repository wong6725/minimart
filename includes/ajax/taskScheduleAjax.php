...
if ( !class_exists( "WCWH_TaskSchedule_AJAX" ) )
{
	
class WCWH_TaskSchedule_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "TaskSchedule_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_task_schedule_action' => false,
            'wh_task_schedule_listing' => false,
            'wh_task_schedule_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_TaskSchedule_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/taskScheduleCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_task_schedule_submission' ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_task_schedule_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_task_schedule_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_task_schedule_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_TaskSchedule_AJAX( $refs );
}
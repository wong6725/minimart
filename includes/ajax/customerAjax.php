...
if ( !class_exists( "WCWH_Customer_AJAX" ) )
{
	
class WCWH_Customer_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Customer_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_customer_action' => false,
            'wh_customer_listing' => false,
            'wh_customer_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_customer_submission' ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_customer_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        if( $succ && $form )
        {
            if( $form['action'] )
            {
                ...
                $outcome['segments']['.template-container #'.$Inst->tplName['print_multi'].'TPL'] = $this->renew_form( $Inst, 'printing_multi_form' );
            }
    ...
    public function wh_customer_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_customer_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

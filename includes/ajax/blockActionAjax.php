...
if ( !class_exists( "WCWH_BlockAction_AJAX" ) )
{
	
class WCWH_BlockAction_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "BlockAction_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_block_action_action' => false,
            'wh_block_action_listing' => false,
            'wh_block_action_form' => false,
        ] );
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_block_action_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'block_action_reference' ) ) )
        {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#block_action_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'block_action_reference':
                        ob_start();
    ...
    public function wh_block_action_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_block_action_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_BlockAction_AJAX( $refs );
}
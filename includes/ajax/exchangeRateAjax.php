...
if ( !class_exists( "WCWH_ExchangeRate_AJAX" ) )
{
	
class WCWH_ExchangeRate_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "ExchangeRate_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_exchange_rate_action' => false,
            'wh_exchange_rate_listing' => false,
            'wh_exchange_rate_form' => false,
            'wh_latest_exchange_rate_listing' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_ExchangeRate_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/exchangeRateCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_exchange_rate_submission' ) return false;
     ...
     *	---------------------------------------------------------------------------------------------------
     */
    public function wh_exchange_rate_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_exchange_rate_listing()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
    	$outcome = array();
    ...
    public function wh_exchange_rate_form()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
    	...
    	if( $succ && $form )
        {
            ...
            if( $result['import_simulate'] && $result['import_key'] )
            {
                $find = [
                    'id' => '{id}',
                    'token' => '{token}',
                ...
                $fields = '<br><input type="hidden" name="'.$key.'[import_key]" value="'.$result['import_key'].'">';
                $replace = [
                    'id' => '',
                    'token' => apply_filters( 'wcwh_generate_token', $section_id ),
                    'hook' => 'wh_exchange_rate_form',
                    'content' => $result['import_simulate'].$fields,
                    'action' => 'confirm-import',
                ];
                $tpl = apply_filters( 'wcwh_get_template_content', 'segment/empty-form.php' );
                $tpl = str_replace( $find, $replace, $tpl );

                $outcome['modal']['title'] = "Preview";
                $outcome['modal']['actionBtn'] = ( $succ )? [ 'close', 'submit' ] : [ 'close' ];
                $outcome['modal']['modal'] = "wcwhModalReview";

                $outcome['content']['#wcwhModalReview .modal-body'] = $tpl;

                $this->Notices->reset_operation_notice();
                if( $succ )
                    $this->Notices->set_notice( 'Import simulation success, preview is needed.', 'success' );
                else
                    $this->Notices->set_notice( 'Import simulation failed, please check!', 'error' );
                $succ = false;
            }
        }
        else
        ...
        if( $log_id && $succ && $form['action'] == 'save' && $result['id'] )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'update', [
                'id'            => $log_id,
                'ref_id'        => $result['id'][0],
    ...
    public function wh_latest_exchange_rate_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-latest-listing-form'] = $this->renew_listing( $Inst, $form, [], 'latest_exchange_rate_listing' );
        $outcome['segments']['#exportExchangeRateTPL'] = $this->renew_form( $Inst, 'export_form' );
...
new WCWH_ExchangeRate_AJAX( $refs );
}
...
if ( !class_exists( "WCWH_PosDO_AJAX" ) )
{
	
class WCWH_PosDO_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PosDO_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_pos_do_action' => false,
            'wh_pos_do_listing' => false,
            'wh_pos_do_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_PosDO_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/posDOCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_pos_do_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_pos_do_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'pos_do_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#pos_do_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'pos_do_reference':
                        ob_start();
    ...
    public function wh_pos_do_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_pos_do_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

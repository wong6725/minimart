...
if ( !class_exists( "WCWH_ToolRequest_AJAX" ) )
{
	
class WCWH_ToolRequest_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "ToolRequest_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_tool_request_action' => false,
            'wh_tool_request_listing' => false,
            'wh_tool_request_form' => false,
            'wh_get_tool_request' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_tool_request_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_tool_request_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_tool_request_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_tool_request_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
    ...
    public function wh_get_tool_request()
    {
        $succ = true;
        $datas = $this->get_submission();
        ...
        if( $succ && $datas )
        {
            ...
            $outcome = $Inst->get_tool_request( $args );
        }
...
new WCWH_ToolRequest_AJAX( $refs );
}
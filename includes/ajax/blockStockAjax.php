...
if ( !class_exists( "WCWH_BlockStock_AJAX" ) )
{
	
class WCWH_BlockStock_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "BlockStock_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_block_stock_action' => false,
            'wh_block_stock_listing' => false,
            'wh_block_stock_form' => false,
        ] );
    }
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_block_stock_action()
    {
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'block_stock_reference' ) ) )
        {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#block_stock_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'block_stock_reference':
                        ob_start();
    ...
    public function wh_block_stock_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_block_stock_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

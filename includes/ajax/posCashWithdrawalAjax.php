...
if ( !class_exists( "WCWH_PosCashWithdrawal_AJAX" ) )
{
	
class WCWH_PosCashWithdrawal_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PosCashWithdrawal_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_pos_cash_withdrawal_action' => false,
            'wh_pos_cash_withdrawal_listing' => false,
            'wh_pos_cash_withdrawal_form' => false,
            'wh_pos_cash_withdrawal_form_update' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_PosCashWithdrawal_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/posCashWithdrawalCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'print', 'wcwh_wh_pos_cash_withdrawal_submission' ] ) ) return false;
     ...
     *	---------------------------------------------------------------------------------------------------
     */
    public function wh_pos_cash_withdrawal_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_pos_cash_withdrawal_listing()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
    	$outcome = array();
    ...
    public function wh_pos_cash_withdrawal_form()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
    ...
    // public function wh_pos_cash_withdrawal_form_update() 
    // {
    //     $this->Notices->reset_operation_notice();
    //     $succ = true;
    ...
    //         {   
    //             ob_start();
    //                 $Inst->gen_form( $data['header']['doc_id']?$data['header']['doc_id']:0, $data, $action);
    //             $outcome['content']['.modal-body'] = ob_get_clean();
    //         }
    //     }
    //     else
...
new WCWH_PosCashWithdrawal_AJAX( $refs );
}
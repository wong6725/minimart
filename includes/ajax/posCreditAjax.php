...
if ( !class_exists( "WCWH_PosCredit_AJAX" ) )
{
	
class WCWH_PosCredit_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PosCredit_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_pos_credit_action' => false,
            'wh_pos_credit_listing' => false,
            'wh_pos_credit_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_POSCredit_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/posCreditCtrl.php" ); 
    }

    public function submission()
    {
		$section_id = 'pos_credit_report';
        $datas = $_REQUEST;
        ...
        $result = $Inst->
        action_handler( $form['action'], $form, $datas );
        if( ! $result['succ'] )
        {
            $succ = false;
     ...
     */
     ...
     public function wh_pos_credit_action()
     {
         $this->Notices->reset_operation_notice();
         $succ = true;
         $outcome = array();
    ...
    public function wh_pos_credit_listing()
    {
        ...
        $outcome['segments']['#exportPosCreditTPL'] = $this->renew_form( $Inst, 'export_form', '' );

        echo json_encode( $outcome );
        die();
    }

    public function wh_pos_credit_form()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;

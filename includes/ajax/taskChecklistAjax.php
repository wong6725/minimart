...
if ( !class_exists( "WCWH_TaskChecklist_AJAX" ) )
{
    
class WCWH_TaskChecklist_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "TaskChecklist_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_task_checklist_action' => false,
            'wh_task_checklist_listing' => false,
            'wh_task_checklist_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_TaskChecklist_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/taskChecklistCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || $datas['action'] != 'wcwh_wh_task_checklist_submission' ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_task_checklist_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $metaForm = array();
         if( !empty( $form['meta_id'] ) )
        {
            parse_str( $form['meta_id'], $metaForm );
            unset( $form['meta_id'] );
        }
    ...
    public function wh_task_checklist_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_task_checklist_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_TaskChecklist_AJAX( $refs );
}
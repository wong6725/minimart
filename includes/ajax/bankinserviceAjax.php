...
if ( !class_exists( "WCWH_BankInService_AJAX" ) )
{
	
class WCWH_BankInService_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "BankInService_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_bankin_service_action' => false,
            'wh_bankin_service_listing' => false,
            'wh_bankin_service_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_BankInService_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/bankinserviceCtrl.php" ); 
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_bankin_service_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_bankin_service_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        if( $succ && $form )
        {
            if( $form['action'] )
            {
                ...
                switch( $action )
                {
                    ...
                    break;
                    case 'bankin_service_reference':
                        ob_start();
                            $Inst->gen_form( $form['id'] );
                        $outcome['content']['.modal-body'] = ob_get_clean();
                    break;
                }
                $outcome['segments']['.template-container #'.$Inst->tplName['bis'].'TPL'] = $this->renew_form( $Inst, 'bis_form' );
                $outcome['segments']['.template-container #'.$Inst->tplName['multiBIS'].'TPL'] = $this->renew_form( $Inst, 'multiBIS_form' );
            }
    ...
    public function wh_bankin_service_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_bankin_service_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        if( $succ && $form )
        {
            ...
            {   
                ...
                if($result['modal'] && $form['action']=='update_api')
                {
                    ...
                    ob_start();
                    $content = apply_filters( 'wcwh_get_template', 'form/bankinservice-form.php',$args);
...
new WCWH_BankInService_AJAX( $refs );
}
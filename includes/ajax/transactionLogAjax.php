...
if ( !class_exists( "WCWH_TransactionLog_AJAX" ) )
{
	
class WCWH_TransactionLog_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "TransactionLog_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_transaction_log_rpt_action' => false,
            'wh_transaction_log_rpt_listing' => false,
            'wh_transaction_log_rpt_form' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_TransactionLog_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/transactionLogCtrl.php" ); 
    }

    public function submission()
    {
		$section_id = 'transaction_log_report';
        $datas = $_REQUEST;
     ...
     */
     ...
     public function wh_transaction_log_rpt_action()
     {
         $this->Notices->reset_operation_notice();
         $succ = true;
         $outcome = array();
    ...
    public function wh_transaction_log_rpt_listing()
    {
        ...
        $outcome['segments']['#exportTransactionLogTPL'] = $this->renew_form( $Inst, 'export_form', '' );

        echo json_encode( $outcome );
        die();
    }

    public function wh_transaction_log_rpt_form()
    {
    	$this->Notices->reset_operation_notice();
    	$succ = true;
...
new WCWH_TransactionLog_AJAX( $refs );
}
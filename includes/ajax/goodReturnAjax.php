...
if ( !class_exists( "WCWH_GoodReturn_AJAX" ) )
{
    
class WCWH_GoodReturn_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "GoodReturn_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_good_return_action' => false,
            'wh_good_return_listing' => false,
            'wh_good_return_form' => false,
            'wh_good_return_by_do_listing' => false,
            'wh_good_return_by_gr_listing' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_good_return_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_good_return_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'good_return_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#good_return_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'good_return_reference':
                        ob_start();
                            $Inst->gen_form( $form['id'] );
                        $outcome['content']['.modal-body'] = ob_get_clean();
                        $outcome['modal']['title'] = 'New Good Return';
                    break;
                }
            }
    ...
    public function wh_good_return_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_good_return_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_good_return_by_do_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'_ref-listing-form'] = $this->renew_listing( $Inst, $form, [], 'view_ref_do_listing' );
    ...
    public function wh_good_return_by_gr_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'_ref-listing-form'] = $this->renew_listing( $Inst, $form, [], 'view_ref_gr_listing' );

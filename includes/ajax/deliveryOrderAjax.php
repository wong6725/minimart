...
if ( !class_exists( "WCWH_DeliveryOrder_AJAX" ) )
{
	
class WCWH_DeliveryOrder_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "DeliveryOrder_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_delivery_order_action' => false,
            'wh_delivery_order_listing' => false,
            'wh_delivery_order_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_delivery_order_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_delivery_order_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'delivery_order_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#delivery_order_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'delivery_order_reference':
                        ob_start();
    ...
    public function wh_delivery_order_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_delivery_order_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_DeliveryOrder_AJAX( $refs );
}
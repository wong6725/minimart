...
if ( !class_exists( "WCWH_PurchaseOrder_AJAX" ) )
{
	
class WCWH_PurchaseOrder_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "PurchaseOrder_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_purchase_order_action' => false,
            'wh_purchase_order_listing' => false,
            'wh_purchase_order_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'wcwh_wh_purchase_order_submission', 'print' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_purchase_order_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'purchase_order_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
        ...
        if( $succ && $form )
        {
            if( $form['action'] )
            {
                ...
                switch( $action )
                {
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#purchase_order_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    ...
                    case $section_id.'_transient_save':
                        ob_start();
                            $Inst->gen_form('','save');
                        $outcome['content']['.modal-body'] = ob_get_clean();
                    break;
                    case $section_id.'_transient_update':
                        ob_start();
                            $Inst->gen_form('','update');
                        $outcome['content']['.modal-body'] = ob_get_clean();
                    break;
                    //----- Form Restoration 09/11/22
                }
    ...
    public function wh_purchase_order_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_purchase_order_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        //----- Form Restoration 09/11/22
        if( ! $this->create_form_transient( $section_id, $form ) )
        {
            $succ = false;
        }
        //----- Form Restoration 09/11/22
        ...
        if( $succ )
        {
            ...
            //----- Form Restoration 09/11/22
            $this->delete_form_transient( $section_id, $form['action'] );
            //----- Form Restoration 09/11/22
        }

        $outcome['segments']['#'.$section_id.'_transient'] = $this->renew_fragment_button( $Inst, 'transient', true, 'col-md-2 rightered', $section_id.'_transient');
        //----- Form Restoration 09/11/22
...
new WCWH_PurchaseOrder_AJAX( $refs );
}
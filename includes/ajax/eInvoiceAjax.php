...
if ( !class_exists( "WCWH_EInvoice_AJAX" ) )
{
	
class WCWH_EInvoice_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "EInvoice_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_e_invoice_action' => false,
            'wh_e_invoice_listing' => false,
            'wh_e_invoice_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'print', 'wcwh_wh_e_invoice_submission' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_e_invoice_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'e_invoice_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#e_invoice_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
    ...
    public function wh_e_invoice_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_e_invoice_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;

...
if ( !class_exists( "WCWH_TransferOrder_AJAX" ) )
{
	
class WCWH_TransferOrder_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "TransferOrder_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'wh_transfer_order_action' => false,
            'wh_transfer_order_listing' => false,
            'wh_transfer_order_form' => false,
        ] );
    }
    ...
    public function submission()
    {
        ...
        if( ! isset( $datas['action'] ) || ! in_array( $datas['action'], [ 'print', 'wcwh_wh_transfer_order_submission' ] ) ) return false;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function wh_transfer_order_action()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Log Activity
        $log_id = 0;
        if( ! in_array( strtolower( $form['action'] ), array( 'edit', 'view', 'transfer_order_reference' ) ) )
        {
            $log_id = apply_filters( 'wcwh_log_activity', 'save', [
                'wh_code'       => $datas['wh'],
                'section'       => $section_id,
                    ...
                    default:
                        ...
                        else
                        {
                            ...
                            if( method_exists( $Inst, 'view_reference' ) )
                                $outcome['segments']['#transfer_order_reference_content'] = $this->segmentify( $Inst, 'view_reference' );
                        }
                    break;
                    case 'transfer_order_reference':
                        ob_start();
    ...
    public function wh_transfer_order_listing()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
    ...
    public function wh_transfer_order_form()
    {
        $this->Notices->reset_operation_notice();
        $succ = true;
...
new WCWH_TransferOrder_AJAX( $refs );
}
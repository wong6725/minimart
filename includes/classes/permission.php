...
if ( !class_exists( "WCWH_Permission_Class" ) ) 
{

class WCWH_Permission_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_permission";

	protected $tbl = "permission";
	...
	public $Notices;
	public $className = "Permission_Class";
    ...
    public function get_permission( $role = '', $user = 0, $scheme_type = "max", $single = true )
    {
        ...
        if( $role || $user )
        {
            $cond.= "AND ( ";
            $cd = array();
            if( $role ) $cd[] = $wpdb->prepare( "a.scheme = %s ", 'role' );
            if( $user ) $cd[] = $wpdb->prepare( "a.scheme = %s ", 'user' );
            if( $cd ) $cond.= implode( ' OR ', $cd );
            $cond.= " ) ";

            $cond.= "AND ( ";
            $cd = array();
            if( $role ) $cd[] = $wpdb->prepare( "a.ref_id = %s ", $role );
            if( $user ) $cd[] = $wpdb->prepare( "a.ref_id = %s ", $user );
            if( $cd ) $cond.= implode( ' OR ', $cd );
            $cond.= " ) ";
        }

        switch( strtolower( $scheme_type ) )
        {
            case 'max':

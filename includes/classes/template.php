...
if ( !class_exists( "WCWH_Template_Class" ) ) 
{

class WCWH_Template_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_template";

	protected $tbl = "templates";
	...
	public $Notices;
	public $className = "Template_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( isset( $filters['tpl_code'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.tpl_code = %s ", $filters['tpl_code'] );
		}
		if( isset( $filters['s'] ) )
		{
            ...
            foreach( $search as $kw )
            {
                $kw = trim( $kw );
                $cd = array();
                $cd[] = "a.tpl_code LIKE '%".$kw."%' ";
				$cd[] = "a.tpl_path LIKE '%".$kw."%' ";
				$cd[] = "a.tpl_file LIKE '%".$kw."%' ";
	...
	public function get_suitable_template( $tpl_code = '', $args = array() )
	{
		...
		$cond = "";
		$grp = "";
		$ord = "ORDER BY a.from_date DESC ";

		$cond.= $wpdb->prepare( "AND a.tpl_code = %s ", $tpl_code );
		$cond.= $wpdb->prepare( "AND a.from_date <= %s ", current_time( 'Y-m-d' ) );
		$cond.= $wpdb->prepare( "AND ( a.to_date >= %s OR a.to_date = %s ) ", current_time( 'Y-m-d' ), "0000-00-00 00:00:00" );

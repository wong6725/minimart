...
if ( !class_exists( "WCWH_Brand_Class" ) ) 
{

class WCWH_Brand_Class extends WCWH_CRUD_Controller 
{
	...
	public function update_metas( $id, $metas )
	{
		...
		foreach( $metas as $key => $value )
		{
			if( is_array( $value ) )
			{
				delete_brand_meta( $id, $key );
				foreach( $value as $val )
				{
					add_brand_meta( $id, $key, $val );
				}
			}
			else
			{
				update_brand_meta( $id, $key, $value );
			}
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isParent )
		{
			$field.= ", prt.brand_no AS prt_brand_no, prt.code AS prt_code, prt.name AS prt_name ";
		...
		if( isset( $filters['brand_no'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.brand_no = %s ", $filters['brand_no'] );
		}
		...
		if( isset( $filters['s'] ) )
		{
            ...
            foreach( $search as $kw )
            {
                $kw = trim( $kw );
                $cd = array();
                $cd[] = "a.brand_no LIKE '%".$kw."%' ";
				$cd[] = "a.code LIKE '%".$kw."%' ";
				$cd[] = "a.name LIKE '%".$kw."%' ";
				
				if( $isParent )
				{
					$cd[] = "prt.brand_no LIKE '%".$kw."%' ";
					$cd[] = "prt.code LIKE '%".$kw."%' ";
					$cd[] = "prt.name LIKE '%".$kw."%' ";
				}
				if( $isCompany )

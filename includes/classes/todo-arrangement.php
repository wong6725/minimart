...
if ( !class_exists( "WCWH_TodoArrangement_Class" ) ) 
{

class WCWH_TodoArrangement_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_arrangement";

	protected $tbl = "todo_arrangement";
	...
	public $Notices;
	public $className = "TodoArrangement_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		$field.= ", b.desc as section_title ";
		$table.= "LEFT JOIN {$this->tables['section']} b ON b.section_id = a.section ";
		...
		if( isset( $filters['match_status'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.match_status = %d ", $filters['match_status'] );
		}
		if( isset( $filters['match_proceed'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.match_proceed = %d ", $filters['match_proceed'] );
		}
		if( isset( $filters['match_halt'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.match_halt = %d ", $filters['match_halt'] );
		}
		if( isset( $filters['action_type'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.action_type = %s ", $filters['action_type'] );
		}
		if( isset( $filters['s'] ) )
		{

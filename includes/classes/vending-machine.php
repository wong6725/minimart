...
if ( !class_exists( "WCWH_VendingMachine_Class" ) ) 
{

class WCWH_VendingMachine_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_vending_machine";

	protected $tbl = "vending_machine";
	...
	public $Notices;
	public $className = "VendingMachine_Class";
	...
	public function update_metas( $id, $metas )
	{
		...
		foreach( $metas as $key => $value )
		{
			if( is_array( $value ) )
			{
				delete_vending_machine_meta( $id, $key );
				foreach( $value as $val )
				{
					add_vending_machine_meta( $id, $key, $val );
				}
			}
			else
			{
				update_vending_machine_meta( $id, $key, $value );
			}
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isWh )
		{
			$field.= ", wh.id AS wh_id, wh.name AS wh_name, wh.code AS wh_code, wh.comp_id AS wh_comp_id ";
			$table.= "LEFT JOIN {$dbname}{$this->tables['warehouse']} wh ON wh.code = a.warehouse_id ";
		}
		...
		{
			$cond.= $wpdb->prepare( "AND a.comp_id = %d ", $filters['comp_id'] );
		}
	    ...
	    if( $args['meta'] )
		{
			foreach( $args['meta'] as $meta_key )
			{
				...
				$table.= $wpdb->prepare( "LEFT JOIN {$dbname}{$this->tables['meta']} {$meta_key} ON {$meta_key}.vending_machine_id = a.id AND {$meta_key}.meta_key = %s ", $meta_key );

...
if ( !class_exists( "WCWH_Section_Class" ) ) 
{

class WCWH_Section_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_section";

	protected $tbl = "section";
	...
	public $Notices;
	public $className = "Section_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		{
			$cond.= $wpdb->prepare( "AND a.table = %s ", $filters['table'] );
		}
		if( isset( $filters['desc'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.desc = %s ", $filters['desc'] );
		}
		if( isset( $filters['s'] ) )
		{

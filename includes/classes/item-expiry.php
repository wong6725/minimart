...
if ( !class_exists( "WCWH_ItemExpiry_Class" ) ) 
{

class WCWH_ItemExpiry_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_item_expiry";

	protected $tbl = "item_expiry";
	...
	public $Notices;
	public $className = "ItemExpiry_Class";
	...
	public function get_expiry( $start_date = '', $item_id = 0 )
	{
		if( ! $item_id ) return false;

		$start_date = ( $start_date )? date( 'Y-m-d', strtotime( $start_date ) ) : date( 'Y-m-d', time() );

		$shelf_life = $this->get_shelf_life( [ 'item_id'=>$item_id ], [ 'single'=>true ] );
		if( $shelf_life && $shelf_life['shelf_life'] )
		{
			return date( 'Y-m-d', strtotime( $start_date . $shelf_life['shelf_life'] ) );
		}

		return false;
	}

	public function get_shelf_life( $filters = [], $args = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();
		...
		$field = "a.id, a.name AS item, cat.term_id AS cat_id, cat.name AS item_category, t.level
			, ie.id AS expiry_id, ie.effective_date, ie.shelf_life ";
		
		$table = "{$dbname}{$this->tables['items']} a ";

		$subsql = "SELECT c.term_id 
			FROM {$dbname}{$this->tables['category_tree']} ct 
			LEFT JOIN {$dbname}{$this->tables['category']} c ON c.term_id = ct.ancestor 
			WHERE ct.descendant = a.category
			ORDER BY ct.level DESC ";
		$table.= "LEFT JOIN {$dbname}{$this->tables['category']} cat ON cat.term_id IN ( {$subsql} ) ";
		$table.= "LEFT JOIN {$dbname}{$this->tables['category_tree']} t ON t.descendant = a.category AND t.ancestor = cat.term_id ";

		$table.= "LEFT JOIN {$dbname}{$this->tables['main']} ie ON ( (ie.ref_id = a.id AND ie.scheme = 'item') OR (ie.ref_id = cat.term_id AND ie.scheme = 'item_category') ) AND ie.status > 0 AND ie.flag > 0 ";
		$cond = "AND ie.id > 0 ";
		$grp = "";
		$ord = "ORDER BY ie.scheme_lvl DESC, t.level ASC ";
		$l = "";
		...
		if( $args['single'] && count( $results ) > 0 )
		{
			$results = $results[0];
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( isset( $filters['category_id'] ) && isset( $filters['item_id'] ) )
		{
			$cond.= "AND ( a.ref_id IN ( '" .implode( "','", $filters['category_id'] ). "' ) OR a.ref_id IN ( '" .implode( "','", $filters['item_id'] ). "' ) ) ";
		}
		else if( isset( $filters['category_id'] ) && ! isset( $filters['item_id'] ) )
		{
			$cond.= "AND a.scheme = 'item_category' AND a.ref_id IN ( '" .implode( "','", $filters['category_id'] ). "' ) ";
		}
		else if( ! isset( $filters['category_id'] ) && isset( $filters['item_id'] ) )
		{
			$cond.= "AND a.scheme = 'item' AND  a.ref_id IN ( '" .implode( "','", $filters['item_id'] ). "' ) ";
		}

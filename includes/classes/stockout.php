...
if ( !class_exists( "WCWH_Stockout_Class" ) ) 
{

class WCWH_Stockout_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_stockout";

	protected $tbl = "stockout_method";
	...
	public $Notices;
	public $className = "Stockout_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		{
			$cond.= $wpdb->prepare( "AND a.order_type = %s ", $filters['order_type'] );
		}
		if( isset( $filters['s'] ) )
		{

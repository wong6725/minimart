...
if ( !class_exists( "WCWH_PriceRef" ) )
{

class WCWH_PriceRef extends WCWH_CRUD_Controller
{
    protected $section_id = "wh_pricing";

    protected $tbl = "price_ref";
    ...
    public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
    {
        ...
        if( $isWH )
        {
            ...
            $table.= "LEFT JOIN {$dbname}{$this->tables['warehouse']} wh ON wh.code = a.seller ";
        }

        $isClient = ( $args && $args['client'] )? true : false;
        if( $isClient )
        {
            $field.= ", cl.id AS client_id, cl.client_no AS client_no, cl.code AS client_code, cl.name AS client_name ";
            $table.= "LEFT JOIN {$dbname}{$this->tables['client']} cl ON cl.code = a.ref_id AND a.scheme = 'client_code' ";
        }

        $isScheme = ( $args && $args['scheme'] )? true : false;
        if( $isScheme )
        {
            $field.= ", sc.id AS scheme_id, sc.title as scheme_title ";
            $table.= "LEFT JOIN {$dbname}{$this->tables['scheme']} sc ON sc.scheme = a.scheme AND sc.section = 'pricing' ";
        }
        ...
        if( isset( $filters['price_id'] ) )
        {
            if( is_array( $filters['price_id'] ) )
                ...
                $cond.= $wpdb->prepare( "AND a.price_id = %s ", $filters['price_id'] );
        }
        if( isset( $filters['not_price_id'] ) )
        {
            if( is_array( $filters['not_price_id'] ) )
                ...
                $cond.= $wpdb->prepare( "AND a.price_id != %s ", $filters['not_price_id'] );
        }
        if( isset( $filters['seller'] ) )
        {

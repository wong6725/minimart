...
class WC_InventoryTrans extends WCWH_CRUD_Controller
{
	...
	public function __construct() 
	{
		...
		$this->_tbl_inventory = $prefix."transaction";
		$this->_tbl_inventory_item = $prefix."transaction_items";
		$this->_tbl_inventory_ref = $prefix."transaction_out_ref";
		$this->_tbl_inventory_meta = $prefix."transaction_meta";
		$this->_tbl_conversion_item = $prefix."transaction_conversion";
		$this->_tbl_weighted_item = $prefix."transaction_weighted";
	...
	private function init_hooks() {
		...
		//add_shortcode( 'testing_inventory' , 'testing4' );
	...
	public function inventory_transaction_data_handle ( $action , $doc_type, $doc_id , $arr_item = array() )
	{
		...
		switch( $doc_type )
		{
			...
			case 'good_return':
				$plus_sign = "-";
				//$direct_fld = ", b.ref_doc_id, b.ref_item_id , '".$plus_sign."' as item_plus ";
				$direct_fld = ", 0 as ref_doc_id, 0 as ref_item_id , '".$plus_sign."' as item_plus ";
			...
			case 'pos_transactions':
				$plus_sign = "*";
				//$direct_fld = ", b.ref_doc_id, b.ref_item_id , e.meta_value as item_plus ";
		...
		$field.= ", prod._stock_out_type AS stock_out_type {$direct_fld}, g.meta_value as item_number, h.meta_value AS stock_item_id
		...
		$header_column = array ( 'doc_id', 'docno', 'doc_type', 'doc_post_date' , 'plus_sign' );
		...
		foreach( $deleted_items as $ditem )
		{
			...
			switch( $ditem['plus_sign'] )
			{
				case "-"; //Stk Out revert +
					...
					$ditem['bal_unit'] = round( $bal_unit + $ditem['unit'], 3 );
					$ditem['bal_amount'] = round( $bal_amount + $ditem['amount'], 2 );
					$ditem['bal_price'] = round( $ditem['bal_amount'] / $ditem['bal_qty'], 5 );
				break;
				case "+"; //Stk In revert -
					$ditem['price'] = $ditem['unit_price'];
					$ditem['amount'] = $ditem['total_price'];
...
//----------------------
	...
	public function update_fifo_document_item ( $doc_post_date , $ditem = array() , $os_qty = 0, $os_unit = 0 , $new_flag = false )
	{
		...
		if( $outstanding_qty > 0 ) //Remain Qty > 0 = No enough stocks
		{
			...
			$prdt = apply_filters( 'wcwh_get_item', [ 'id'=>$ditem['product_id'] ], [], true, [] );
			$note = ( $prdt )? "Item {$prdt['name']} - {$prdt['code']} " : "";
			if( $this->Notices ) $this->Notices->set_notice( $note."Insufficient stock", "error", $this->className."|update_fifo_document_item" );
		}
	...
	public function offset_fifo_document_item ( $ditem = array() , $os_qty = 0, $os_unit = 0 , $reallocated = false )
	{
		...
		if( $outstanding_offset_qty > 0 )
		{
			...
			$prdt = apply_filters( 'wcwh_get_item', [ 'id'=>$ditem['product_id'] ], [], true, [] );
			$note = ( $prdt )? "Item {$prdt['name']} - {$prdt['code']} " : "";
			if( $this->Notices ) $this->Notices->set_notice( $note."Insufficient stocks", "error", $this->className."|offset_fifo_document_item" );
		}

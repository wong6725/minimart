...
if ( !class_exists( "WCWH_Client_Class" ) ) 
{

class WCWH_Client_Class extends WCWH_CRUD_Controller 
{
	...
	public function update_metas( $id, $metas )
	{
		...
		foreach( $metas as $key => $value )
		{
			if( is_array( $value ) )
			{
				delete_client_meta( $id, $key );
				foreach( $value as $val )
				{
					add_client_meta( $id, $key, $val );
				}
			}
			else
			{
				update_client_meta( $id, $key, $value );
			}
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isParent )
		{
			$field.= ", prt.client_no AS prt_client_no, prt.code AS prt_code, prt.name AS prt_name ";
		...
		if( isset( $filters['client_no'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.client_no = %s ", $filters['client_no'] );
		}
		...
		if( isset( $filters['s'] ) )
		{
            ...
            foreach( $search as $kw )
            {
                $kw = trim( $kw );
                $cd = array();
                $cd[] = "a.client_no LIKE '%".$kw."%' ";
				$cd[] = "a.code LIKE '%".$kw."%' ";
				$cd[] = "a.name LIKE '%".$kw."%' ";
				
				if( $isParent )
				{
					$cd[] = "prt.client_no LIKE '%".$kw."%' ";
					$cd[] = "prt.code LIKE '%".$kw."%' ";
					$cd[] = "prt.name LIKE '%".$kw."%' ";
				}
				if( $isCompany )
	    ...
	    if( $args['meta'] )
		{
			foreach( $args['meta'] as $meta_key )
			{
				...
				$table.= $wpdb->prepare( "LEFT JOIN {$dbname}{$this->tables['meta']} {$meta_key} ON {$meta_key}.client_id = a.id AND {$meta_key}.meta_key = %s ", $meta_key );

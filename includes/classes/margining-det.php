...
if ( !class_exists( "WCWH_MarginingDet" ) )
{

class WCWH_MarginingDet extends WCWH_CRUD_Controller
{
    protected $section_id = "wh_margining";

    protected $tbl = "margining_det";
    ...
    public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
    {
        ...
        $isClient = ( $args['client'] )? 1 : 0;
        if( $isClient )
        {
            $field.= ", c.id AS client_id, c.code AS client_code, c.name AS client_name ";
            $table.= "LEFT JOIN {$dbname}{$this->tables['client']} c ON c.code = a.client ";
        }
        ...
        if( isset( $filters['mg_id'] ) )
        {
            if( is_array( $filters['mg_id'] ) )
                $cond.= "AND a.mg_id IN ('" .implode( "','", $filters['mg_id'] ). "') ";
            else
                $cond.= $wpdb->prepare( "AND a.mg_id = %d ", $filters['mg_id'] );
        }
        if( isset( $filters['not_mg_id'] ) )
        {
            if( is_array( $filters['not_mg_id'] ) )
                $cond.= "AND a.mg_id NOT IN ('" .implode( "','", $filters['not_mg_id'] ). "') ";
            else
                $cond.= $wpdb->prepare( "AND a.mg_id != %d ", $filters['not_mg_id'] );
        }

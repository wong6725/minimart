...
if ( !class_exists( "WCWH_PromoDetail" ) )
{

class WCWH_PromoDetail extends WCWH_CRUD_Controller
{
    protected $section_id = "wh_promo";

    protected $tbl = "promo_detail";
    ...
    public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
    {
        ...
        $isItem = ( $args['item'] )? 1 : 0;
        if( $isItem )
        {
            $field.= ", i.code AS prdt_code, i.name AS prdt_name, i._uom_code AS prdt_uom ";
        ...
        if( isset( $filters['promo_id'] ) )
        {
            if( is_array( $filters['promo_id'] ) )
                $cond.= "AND a.promo_id IN ('" .implode( "','", $filters['promo_id'] ). "') ";
            else
                $cond.= $wpdb->prepare( "AND a.promo_id = %d ", $filters['promo_id'] );
        }
        if( isset( $filters['not_promo_id'] ) )
        {
            if( is_array( $filters['not_promo_id'] ) )
                $cond.= "AND a.promo_id NOT IN ('" .implode( "','", $filters['not_promo_id'] ). "') ";
            else
                $cond.= $wpdb->prepare( "AND a.promo_id != %d ", $filters['not_promo_id'] );
        }
        ...
        {
            $cond.= $wpdb->prepare( "AND a.match = %s ", $filters['match'] );
        }

...
if ( !class_exists( "WCWH_PosOrder_Class" ) ) 
{

class WCWH_PosOrder_Class extends WCWH_CRUD_Controller 
{
	...
	public $Notices;
	public $className = "PosOrder_Class";
	...
	public function get_session( $cond = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();
		...
		foreach( $cond as $k => $v )
		{
			$cd.= $wpdb->prepare( "AND {$k} = %s ", $v );
		}

		$sql = "SELECT * FROM {$this->tables['pos_sessions']} WHERE 1 {$cd};";
	...
	public function update_session( $cond = [], $params = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();
		...
		$update = $wpdb->update( $this->tables['pos_sessions'], $params, $cond );
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		$field = "a.ID AS id, ma.meta_value AS order_no, a.post_date AS order_date, a.post_status AS order_status, a.post_author AS created 
			...
			, IF( tr.meta_value > 0, 'tool', 'grocery' ) AS order_type 
		...
		$table.= "LEFT JOIN {$dbname}{$this->tables['ordermeta']} ip ON ip.post_id = a.ID AND ip.meta_key = 'ip_address' ";
		...
		$cond.= "AND ( cd.meta_value IS NULL OR cd.meta_value = '' ) ";
	...
	public function count_statuses()
	{
		...
		$cond = $wpdb->prepare( "AND a.post_type = %s AND a.post_status != %s AND ( cd.meta_value IS NULL OR cd.meta_value = '' ) ", "shop_order", "-1" );

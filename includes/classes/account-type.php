...
if ( !class_exists( "WCWH_AccountType_Class" ) ) 
{

class WCWH_AccountType_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_account_type";

	protected $tbl = "customer_acc_type";
	...
	public $Notices;
	public $className = "AccountType_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isCGroup )
		{
			...
			$table.= "LEFT JOIN {$dbname}{$this->tables['customer_group']} cgroup ON cgroup.id = a.def_cgroup_id ";
		}

		$isTerm = ( $args && $args['term'] )? true : false;
		if( $isTerm )
		{
			$field.= ", ct.name AS term_name, ct.days AS term_days, ct.type AS term_type ";
			$table.= "LEFT JOIN {$dbname}{$this->tables['credit_term']} ct ON ct.id = a.term_id ";
		}

		$isAutoTopup = ( $args && $args['auto_topup'] )? true : false;
		if( $isAutoTopup )
		{
			$cond.= $wpdb->prepare( "AND a.auto_topup = %d ", 1 );
		}
		...
		{
			$cond.= $wpdb->prepare( "AND a.plant = %s ", $filters['plant'] );
		}
		if( isset( $filters['s'] ) )
		{
		...
		$isSAP = ( $args && $args['sap_only'] )? true : false;
		if( $isSAP )
		{
			$cond.= "AND ( a.employee_prefix IS NOT NULL AND a.employee_prefix != '' ) ";
		}

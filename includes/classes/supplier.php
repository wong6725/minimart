...
if ( !class_exists( "WCWH_Supplier_Class" ) ) 
{

class WCWH_Supplier_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_supplier";

	protected $tbl = "supplier";
	...
	public $Notices;
	public $className = "Supplier_Class";
	...
	public function update_metas( $id, $metas )
	{
		...
		foreach( $metas as $key => $value )
		{
			if( is_array( $value ) )
			{
				delete_supplier_meta( $id, $key );
				foreach( $value as $val )
				{
					add_supplier_meta( $id, $key, $val );
				}
			}
			else
			{
				update_supplier_meta( $id, $key, $value );
			}
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isParent )
		{
			$field.= ", prt.supplier_no AS prt_supplier_no, prt.code AS prt_code, prt.name AS prt_name ";
		...
		if( isset( $filters['supplier_no'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.supplier_no = %s ", $filters['supplier_no'] );
		}
		...
		if( isset( $filters['s'] ) )
		{
            ...
            foreach( $search as $kw )
            {
                $kw = trim( $kw );
                $cd = array();
                $cd[] = "a.supplier_no LIKE '%".$kw."%' ";
				$cd[] = "a.code LIKE '%".$kw."%' ";
				$cd[] = "a.name LIKE '%".$kw."%' ";
				
				if( $isParent )
				{
					$cd[] = "prt.supplier_no LIKE '%".$kw."%' ";
					$cd[] = "prt.code LIKE '%".$kw."%' ";
					$cd[] = "prt.name LIKE '%".$kw."%' ";
				}
				if( $isCompany )
	    ...
	    if( $args['meta'] )
		{
			foreach( $args['meta'] as $meta_key )
			{
				...
				$table.= $wpdb->prepare( "LEFT JOIN {$dbname}{$this->tables['meta']} {$meta_key} ON {$meta_key}.supplier_id = a.id AND {$meta_key}.meta_key = %s ", $meta_key );
		...
		if( isset( $filters['self_bill'] ) && $filters['self_bill'] != '' )
        {   
            $cond.= $wpdb->prepare( "AND a.self_bill = %d ", $filters['self_bill'] );
        }
	...
	public function get_export_data( $filters = array(), $single = false )
	{
		...
		$field = "a.code, a.name, a.supplier_no, b.code AS parent, a.status, b.flag ";

		$table = "{$this->tables['main']} a ";
		$table.= "LEFT JOIN {$this->tables['main']} b ON b.id = a.parent ";
		$cond = "";
		$grp = "";
		$ord = "";
		$l = "";

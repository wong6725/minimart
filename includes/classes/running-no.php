...
if ( !class_exists( "WCWH_RunningNo_Class" ) ) 
{

class WCWH_RunningNo_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_runningno";

	protected $tbl = "doc_runningno";
	...
	public $Notices;
	public $className = "RunningNo_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		{
			$cond.= $wpdb->prepare( "AND a.prefix = %s ", $filters['prefix'] );
		}
		if( isset( $filters['suffix'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.suffix = %s ", $filters['suffix'] );
		}
		if( isset( $filters['s'] ) )
		{
	...
	public function generate_docno( $sdocno, $doc_type = '', $def_prefix = '', $args = array() )
	{
		...
		if ( null !== $runningno ) 
		{
			switch( $runningno['type'] )
			{
				...
				break;
				case 'hw_code':
					$sdocno = hw_code_gen( (int)$runningno['prefix'], (int)$runningno['suffix'] );
				break;
			}
		}
		...
		if ( null !== $runningno ) 
		{
			switch( $runningno['type'] )
			{
				...
				default:
					...
					if( $runningno['length'] > 0 )
					{
						$docno = ( $sdocno )? substr( $sdocno, $runningno['length'] * -1 ) : $sdocno;
						$sdocno = $prefix.$docno.$runningno['suffix'];
					}
					else
					{
						$sdocno = $prefix.$runningno['suffix'];
					...
					$sdocno = apply_filters( 'wcwh_docno_replacer', $sdocno, $doc_type );
				break;
				case 'random':
					$sdocno = generateSerial( $runningno['length'] );
				break;

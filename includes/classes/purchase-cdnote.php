...
if ( !class_exists( "WCWH_PurchaseCDNote_Class" ) ) 
{

class WCWH_PurchaseCDNote_Class extends WC_DocumentTemplate 
{
	...
	public $Notices;
	public $className = "PurchaseCDNote_Class";

	private $doc_type = [
		'1' => 'purchase_credit_note',
		'2' => 'purchase_debit_note',
	];
	...
	public function child_action_handle( $action , $header = array() , $details  = array() )
	{
		...
		switch ( $action )
		{
			...
			case "post": 
				...
				$need_transact = false;
				$detail_items = $this->get_detail( [ 'doc_id'=>$doc_id ], [], false, [ 'usage'=>1, 'meta'=>['total_amount'] ] );
				if( $detail_items )
				{
				...
				if( isset( $doc_id ) && $need_transact && ! $need_inv )
				{
					...
					$succ = apply_filters( 'warehouse_inventory_transaction_filter', 'save', $header_item['doc_type'], $doc_id );	
					if( ! $succ )
					{
			...
			case "unpost": 
				...
				$t_exists = apply_filters( 'wcwh_get_exist_inventory_transaction', $doc_id, $header_item['doc_type'] );
				if ( ! $t_exists )
				{
					if( $succ )
					{
				...
				{
					$succ = apply_filters( 'warehouse_inventory_transaction_filter', 'delete', $header_item['doc_type'], $doc_id );
					if( ! $succ )
					{
						$this->Notices->set_notices( apply_filters( 'wcwh_inventory_get_notices', true ) );
					}

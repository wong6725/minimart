...
if ( !class_exists( "WCWH_Criteria_Class" ) ) 
{

class WCWH_Criteria_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_criteria";

	protected $tbl = "criteria";
	...
	public $Notices;
	public $className = "Criteria_Class";
	...
	public function update_metas( $id, $metas )
	{
		...
		foreach( $metas as $key => $value )
		{
			if( is_array( $value ) )
			{
				delete_criteria_meta( $id, $key );
				foreach( $value as $val )
				{
					add_criteria_meta( $id, $key, $val );
				}
			}
			else
			{
				update_criteria_meta( $id, $key, $value );
			}
		}
	...
	public function update_childs_parent( $data, $child_list )
	{
		...
		if( $data && $child_list && empty( $data['ancestor'] ) )
		{
		    ...
		    foreach( $child_list as $child )
		    {
				...
				{
					$succ = false;
					if( $this->Notices ) $this->Notices->set_notice( "update-fail", "error", $this->className."|update_childs_parent" );
				}
		    }
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isParent )
		{
			$field.= ", prt.criteria_no AS prt_criteria_no, prt.code AS prt_code, prt.name AS prt_name ";
		...
		if( isset( $filters['criteria_no'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.criteria_no = %s ", $filters['criteria_no'] );
		}
		...
		if( isset( $filters['s'] ) )
		{
            ...
            foreach( $search as $kw )
            {
                $kw = trim( $kw );
                $cd = array();
                $cd[] = "a.criteria_no LIKE '%".$kw."%' ";
				$cd[] = "a.code LIKE '%".$kw."%' ";
				$cd[] = "a.name LIKE '%".$kw."%' ";
				
				if( $isParent )
				{
					$cd[] = "prt.criteria_no LIKE '%".$kw."%' ";
					$cd[] = "prt.code LIKE '%".$kw."%' ";
					$cd[] = "prt.name LIKE '%".$kw."%' ";
				}
				if( $isCompany )
	    ...
	    if( $args['meta'] )
		{
			foreach( $args['meta'] as $meta_key )
			{
				...
				$table.= $wpdb->prepare( "LEFT JOIN {$dbname}{$this->tables['meta']} {$meta_key} ON {$meta_key}.criteria_id = a.id AND {$meta_key}.meta_key = %s ", $meta_key );

...
class WC_Warehouse_Stocks extends WCWH_CRUD_Controller
{
	...
	public function __construct() 
	{
		...
		$this->_tbl_warehouse_stocks = $prefix."inventory";
		...
		switch ( strtolower( $action ) ){
			...
			case "update":
				foreach ($details as $item) 
				{
					...
					$unit = 0;
					$amt = 0;
					$inv_item = $ditem = wp_parse_args( $item, $this->item_sprice_defaults ); 
					if( isset ( $exists_item[ $item['sales_item_id'] ] ) )
					{
						$exists =  $exists_item[ $item['sales_item_id'] ];
						$inv_item['qty'] = $ditem['qty'] - $exists['qty'];
						$inv_item['unit'] = $ditem['unit'] - $exists['unit'];
						$inv_item['total_amount'] = $ditem['total_amount'] - $exists['total_amount'];
					...
					{	
						$inv_item['qty'] = $ditem['qty'];
						$inv_item['unit'] = $ditem['unit'];
						$inv_item['total_amount'] = $ditem['total_amount'];
					...
					$succ = $this->update_warehouse_stocks_qty( 'sales', $warehouse_id, $strg_id, $item['prdt_id'], $inv_item );
					if( ! $succ )
					{
						break;
					}
			...
			case "delete":
				...
				foreach ($details as $item) 
				{
					$inv_item = wp_parse_args( $item, $this->item_sprice_defaults ); 
					if( isset ( $exists_item[ $item['sales_item_id'] ] ) )
					{
						$exists =  $exists_item[ $item['sales_item_id'] ];
						$inv_item['qty'] =  ( - $exists['qty'] );
						$inv_item['unit'] =  ( - $exists['unit'] );
						$inv_item['total_amount'] =  ( - $exists['total_amount'] );
						$succ = $this->update_warehouse_stocks_qty( 'sales', $warehouse_id, $strg_id, $exists['prdt_id'], $inv_item );
						if( ! $succ )
						{
							break;
						}

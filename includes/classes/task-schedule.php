...
if ( !class_exists( "WCWH_TaskSchedule_Class" ) ) 
{

class WCWH_TaskSchedule_Class extends WC_DocumentTemplate 
{
	protected $section_id = "wh_task_schedule";

	protected $tbl = "document";
	...
	public $Notices;
	public $className = "TaskSchedule_Class";
	...
	public function filterDataChecked($array_one, $array_two)
	{
		foreach ($array_two as $data) {
			...
			foreach ($array_one as $array_one_data) {
				if ($data['doc_id'] === $array_one_data['doc_id'] && $data['doc_date'] === $array_one_data['doc_date'] && array_diff($data['serial2'], $array_one_data['serial2']) == [] && $data['hstatus'] === $array_one_data['hstatus']) 
				{
					$matching = true;
					break;
				}
	...
	{	
		...
		switch ( $action ){
			...
			case "update":
				...
				$data = $this->get_document_header($doc_id);
				
				foreach($details as $key => $value){
					if(isset($value["item_id"]) && empty($value["item_id"])){
						// unset($details[$key]["item_id"]);
						$details[$key]["doc_id"] = $doc_id;
						$details[$key]['lupdate_by'] = $this->user_id;
						$details[$key]['lupdate_at'] = current_time( 'mysql' );
						$details[$key]['created_by'] = $this->user_id;
						$details[$key]['created_at'] = current_time( 'mysql' );
						$details[$key]['strg_id'] = "0";
						$details[$key]['uom_id'] = '';
						$details[$key]['bunit'] = "0";
						$details[$key]['ref_doc_id'] = "0";
						$details[$key]['ref_item_id'] = "0";
						$details[$key]["status"] = "1";

						if($data['status'] == 6){
							$details[$key]["status"] = "6";
						}

						$item_id = $this->add_document_items( $details[$key] );
						$details[$key]["item_id"] = $item_id;
					}
				}
				
				$old_datas = $this->get_checklist([ 'doc_id' => $doc_id, 'bqty' => 'uncheck' ], false);
				$details = array_merge($old_datas, $details);
	...
	public function get_export_data( $filters = array(), $single = false )
	{
		...
		if( !empty( $filters ) )
		{
			...
			if( isset( $filters['docno'] ) && !empty( $filters['docno'] )  )
			{
	...
	public function get_infos($filters = array(), $single = false)
	{
	    global $wcwh;
	    $wpdb = $this->db_wpdb;
	    $prefix = $this->get_prefix();
	    ...
	    $header_meta = ['remark', 'client_company_code', 'recursive_period', 'last_run_date'];
	    if ($header_meta) {
	        foreach ($header_meta as $i => $meta_key) {
	            $k = 'h' . ($i + 1);
	            $field .= ", {$k}.meta_value AS {$meta_key} ";
	    ...
	    $field .= ", '0' AS ref_doc_id, '0' AS ref_item_id, b.status AS dstatus ";

	    $serial2_subquery = "(SELECT CONCAT('[ ', GROUP_CONCAT(DISTINCT CONCAT('\"', IFNULL(meta_value, ''), '\"')), ' ]') FROM {$this->_tbl_document_meta} WHERE doc_id = a.doc_id AND meta_key = 'serial2') AS serial2";
    	$field .= ", {$serial2_subquery}";

	    $table .= "LEFT JOIN {$this->tables['document_items']} b ON b.doc_id = a.doc_id ";

	    $cond .= $wpdb->prepare("AND a.doc_type = %s AND a.status >= %d ", $this->doc_type, 1);

	    if( isset( $filters['from_doc_date'] ) && !empty( $filters['from_doc_date'] )  )
		{
			$cond.= $wpdb->prepare( "AND a.doc_date >= %s ", $filters['from_doc_date'] );
			unset( $filters['from_doc_date'] );
		}

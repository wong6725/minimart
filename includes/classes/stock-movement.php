...
if ( !class_exists( "WCWH_StockMovement_Class" ) )
{

class WCWH_StockMovement_Class extends WCWH_CRUD_Controller
{
    ...
    public function set_db_tables()
    {
        ...
        $this->tables = array(
            "stock_movement"    => $prefix."stock_movement",
...
$fld = "'{$wh}' AS warehouse_id, '{$strg_id}' AS strg_id, '{$month}' AS month, a.product_id
    ...
    , @qty:= IFNULL(a.qty,0)+@gr_qty+@rp_qty+@ti_qty+@dr_qty-@so_qty-@to_qty-@gi_qty-@gt_qty+@adj_qty AS qty
    , @mtr:= IFNULL(a.mtr,0)+@gr_mtr+@rp_mtr+@ti_mtr+@dr_mtr-@so_mtr-@to_mtr-@gi_mtr-@gt_mtr+@adj_mtr AS mtr
    , @amt:= IFNULL(a.amt,0)+@gr_amt+@rp_amt+@ti_amt+@dr_amt-@so_amt-@to_amt-@gi_amt-@gt_amt+@adj_amt AS amt
    ...
    , @pos_amt:= IFNULL( ( @amt / @qty ) * @pos_uom_qty, 0 ) AS pos_amt
    ...
    , IF(@op_amt != 0,@op_amt,@op_amt)+@gr_amt+@rp_amt+@ti_amt+@dr_amt-@so_amt-@to_amt-@gi_amt-@gt_amt-IFNULL(@pos_amt,0)+@adj_amt AS closing_amt ";
        ...
        public function get_opening( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " b.product_id {$fld}
                , SUM( b.closing_qty ) AS op_qty, SUM( b.closing_mtr ) AS op_mtr, SUM( b.closing_amt ) AS op_amt
                , SUM( b.qty ) AS qty, SUM( b.mtr ) AS mtr, SUM( b.amt ) AS amt
        ...
        public function get_purchase_debit_credit( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " d.product_id {$fld}
                , 0 AS op_qty, 0 AS op_mtr, 0 AS op_amt, 0 AS qty, 0 AS mtr, 0 AS amt
                , SUM( IF( ma.meta_value = 2, d.bqty, d.bqty * -1 ) ) AS gr_qty
                , SUM( ROUND( IF( ma.meta_value = 2, d.bunit * -1, d.bunit ), 3 ) ) AS gr_mtr
                , SUM( ROUND( IF( ma.meta_value = 2, dma.meta_value * -1, dma.meta_value ), 2 ) ) AS gr_amt
                , 0 AS rp_qty, 0 AS rp_mtr, 0 AS rp_amt
                , 0 AS ti_qty, 0 AS ti_mtr, 0 AS ti_amt
                , 0 AS dr_qty, 0 AS dr_mtr, 0 AS dr_amt 
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
        ...
        */
        public function get_goods_receipt( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                , 0 AS op_qty, 0 AS op_mtr, 0 AS op_amt, 0 AS qty, 0 AS mtr, 0 AS amt
                , SUM( ti.bqty ) AS gr_qty, SUM( ROUND( ti.bunit, 3 ) ) AS gr_mtr, SUM( ROUND( ti.total_price, 2 ) ) AS gr_amt
                , 0 AS rp_qty, 0 AS rp_mtr, 0 AS rp_amt
                , 0 AS ti_qty, 0 AS ti_mtr, 0 AS ti_amt
                , 0 AS dr_qty, 0 AS dr_mtr, 0 AS dr_amt 
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
...
, IF( mg.id > 0, SUM( ROUND( 
    ROUND( 
        ...
        WHEN mg.round_type = 'ROUND' 
            THEN ROUND( ROUND( ROUND(ti.total_price/ti.bqty,5)*( 1+( IFNULL(mg.margin,0)/100 ) ), 5 ) / IF( mg.round_nearest IS NULL OR mg.round_nearest = 0, 0.01, mg.round_nearest ) ) * IF( mg.round_nearest IS NULL OR mg.round_nearest = 0, 0.01, mg.round_nearest ) 
        WHEN mg.round_type = 'CEIL' 
            THEN CEIL( ROUND( ROUND(ti.total_price/ti.bqty,5)*( 1+( IFNULL(mg.margin,0)/100 ) ), 5 ) / IF( mg.round_nearest IS NULL OR mg.round_nearest = 0, 0.01, mg.round_nearest ) ) * IF( mg.round_nearest IS NULL OR mg.round_nearest = 0, 0.01, mg.round_nearest ) 
        WHEN mg.round_type = 'FLOOR' 
            THEN FLOOR( ROUND( ROUND(ti.total_price/ti.bqty,5)*( 1+( IFNULL(mg.margin,0)/100 ) ), 5 ) / IF( mg.round_nearest IS NULL OR mg.round_nearest = 0, 0.01, mg.round_nearest ) ) * IF( mg.round_nearest IS NULL OR mg.round_nearest = 0, 0.01, mg.round_nearest ) 
        WHEN mg.round_type IS NULL OR mg.round_type = 'DEFAULT' 
            THEN ROUND( ROUND(ti.total_price/ti.bqty,5)*( 1+( IFNULL(mg.margin,0)/100 ) ), 5 ) 
        END
    , 5 ) 
* ti.bqty, 2 ) ), SUM( ROUND( ti.total_price, 2 ) ) ) AS gr_amt 
                , 0 AS rp_qty, 0 AS rp_mtr, 0 AS rp_amt
                , 0 AS ti_qty, 0 AS ti_mtr, 0 AS ti_amt
                , 0 AS dr_qty, 0 AS dr_mtr, 0 AS dr_amt 
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
        ...
        */
        public function get_reprocess( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                , 0 AS op_qty, 0 AS op_mtr, 0 AS op_amt, 0 AS qty, 0 AS mtr, 0 AS amt
                , 0 AS gr_qty, 0 AS gr_mtr, 0 AS gr_amt
                , SUM( ti.bqty ) AS rp_qty, SUM( ROUND( ti.bunit, 3 ) ) AS rp_mtr, SUM( ROUND( ti.total_price, 2 ) ) AS rp_amt 
                , 0 AS ti_qty, 0 AS ti_mtr, 0 AS ti_amt
                , 0 AS dr_qty, 0 AS dr_mtr, 0 AS dr_amt 
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
                , 0 AS to_qty, 0 AS to_mtr, 0 AS to_amt
        ...
        */
        public function get_transfer_item( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                ...
                , 0 AS gr_qty, 0 AS gr_mtr, 0 AS gr_amt
                , 0 AS rp_qty, 0 AS rp_mtr, 0 AS rp_amt
                , SUM( ti.bqty ) AS ti_qty, SUM( ROUND( ti.bunit, 3 ) ) AS ti_mtr, SUM( ROUND( ti.total_price, 2 ) ) AS ti_amt 
                , 0 AS dr_qty, 0 AS dr_mtr, 0 AS dr_amt 
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
                , 0 AS to_qty, 0 AS to_mtr, 0 AS to_amt
                , 0 AS gi_qty, 0 AS gi_mtr, 0 AS gi_amt
        ...
        */
        public function get_do_revise( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                ...
                , 0 AS rp_qty, 0 AS rp_mtr, 0 AS rp_amt
                , 0 AS ti_qty, 0 AS ti_mtr, 0 AS ti_amt 
                , SUM( ti.bqty ) AS dr_qty, SUM( ROUND( ti.bunit, 3 ) ) AS dr_mtr, SUM( ROUND( ti.total_price, 2 ) ) AS dr_amt 
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
                , 0 AS to_qty, 0 AS to_mtr, 0 AS to_amt
                , 0 AS gi_qty, 0 AS gi_mtr, 0 AS gi_amt
                , 0 AS gt_qty, 0 AS gt_mtr, 0 AS gt_amt
        ...
        */
        public function get_sale_delivery_order( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                ...
                , 0 AS ti_qty, 0 AS ti_mtr, 0 AS ti_amt
                , 0 AS dr_qty, 0 AS dr_mtr, 0 AS dr_amt 
                , SUM( ti.bqty ) AS so_qty, SUM( ROUND( ti.bunit, 3 ) ) AS so_mtr, SUM( ROUND( ti.total_cost, 2 ) ) AS so_amt
        ...
        */
        public function get_transfer_delivery_order( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                ...
                , 0 AS dr_qty, 0 AS dr_mtr, 0 AS dr_amt 
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
                , SUM( ti.bqty ) AS to_qty, SUM( ROUND( ti.bunit, 3 ) ) AS to_mtr, SUM( ROUND( ti.total_cost, 2 ) ) AS to_amt 
                , 0 AS gi_qty, 0 AS gi_mtr, 0 AS gi_amt
                , 0 AS gt_qty, 0 AS gt_mtr, 0 AS gt_amt
                , 0 AS pos_qty, 0 AS pos_uom_qty, 0 AS pos_mtr, 0 AS pos_amt, 0 AS pos_sale
                , 0 AS adj_qty, 0 AS adj_mtr, 0 AS adj_amt ";
        ...
        */
        public function get_good_issue( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " IF( ti.product_id, ti.product_id, d.product_id ) AS product_id {$fld}
                ...
                , 0 AS so_qty, 0 AS so_mtr, 0 AS so_amt, 0 AS so_sale, 0 AS so_adj
                , 0 AS to_qty, 0 AS to_mtr, 0 AS to_amt
                , SUM( IF( ti.product_id, ti.bqty, d.bqty ) ) AS gi_qty
                , SUM( ROUND( IF( ti.product_id, ti.bunit, IF( dma.meta_value > 0, dma.meta_value, d.bunit ) ), 3 ) ) AS gi_mtr 
                , SUM( ROUND( IF( ti.product_id, ti.total_cost, d.bqty * dmc.meta_value ), 2 ) ) AS gi_amt 
                , 0 AS gt_qty, 0 AS gt_mtr, 0 AS gt_amt
                , 0 AS pos_qty, 0 AS pos_uom_qty, 0 AS pos_mtr, 0 AS pos_amt, 0 AS pos_sale
                , 0 AS adj_qty, 0 AS adj_mtr, 0 AS adj_amt ";
        ...
        */
        public function get_good_return( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                ...
                , 0 AS to_qty, 0 AS to_mtr, 0 AS to_amt
                , 0 AS gi_qty, 0 AS gi_mtr, 0 AS gi_amt
                , SUM( ti.bqty ) AS gt_qty, SUM( ROUND( ti.bunit, 3 ) ) AS gt_mtr, SUM( ROUND( ti.total_cost, 2 ) ) AS gt_amt 
                , 0 AS pos_qty, 0 AS pos_uom_qty, 0 AS pos_mtr, 0 AS pos_amt, 0 AS pos_sale
                , 0 AS adj_qty, 0 AS adj_mtr, 0 AS adj_amt ";
        ...
        */
        public function get_pos( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " d.meta_value AS product_id {$fld}
                ...
                , SUM( ROUND( IF( ia.meta_value > 0 {$metric}, g.meta_value * e.meta_value, e.meta_value ), 2 ) ) AS pos_uom_qty
        ...
        */
        public function get_adjustment( $filters = [], $run = false, $args = [] )
        {
            ...
            $field = " ti.product_id {$fld}
                ...
                , SUM( ROUND( IF( ti.plus_sign = '-', ti.total_cost * -1, ti.total_price ), 2 ) ) AS adj_amt ";
            ...
            $field = " d.product_id {$fld}
                ...
                , 0 AS gt_qty, 0 AS gt_mtr, 0 AS gt_amt
                , 0 AS pos_qty, 0 AS pos_uom_qty, 0 AS pos_mtr, 0 AS pos_amt, 0 AS pos_sale
                , SUM( IF( dma.meta_value = '-', d.bqty * -1, d.bqty ) ) AS adj_qty 
                , SUM( ROUND( IF( dma.meta_value = '-', dmd.meta_value * -1, dmd.meta_value ), 3 ) ) AS adj_mtr 
                , SUM( ROUND( IF( dma.meta_value = '-', dmc.meta_value * -1, dmc.meta_value ), 2 ) ) AS adj_amt ";
            ...
            $table.= "LEFT JOIN {$dbname}{$this->tables['document_meta']} dmd ON dmd.doc_id = h.doc_id AND dmd.item_id = d.item_id AND dmd.meta_key = 'bunit' ";

            $cond = $wpdb->prepare( "AND h.status >= %d ", 6 );
            $cond.= "AND h.doc_type IN ( 'stock_movement_rectify' ) ";
        ...
        public function get_rectify( $filters = [], $run = false, $args = [] )
        {
            global $wcwh;
            $wpdb = $this->db_wpdb;
            $prefix = $this->get_prefix();

...
if ( !class_exists( "WCWH_Company_Class" ) ) 
{

class WCWH_Company_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_company";

	protected $tbl = "company";
	...
	public $Notices;
	public $className = "Company_Class";
	...
	public function update_metas( $id, $metas )
	{
		...
		foreach( $metas as $key => $value )
		{
			if( is_array( $value ) )
			{
				delete_company_meta( $id, $key );
				foreach( $value as $val )
				{
					add_company_meta( $id, $key, $val );
				}
			}
			else
			{
				update_company_meta( $id, $key, $value );
			}
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isParent )
		{
			$field.= ", prt.custno AS prt_custno, prt.code AS prt_code, prt.name AS prt_name, prt.regno AS prt_regno ";
		...
		//join stage
		$isStage = ( $args && $args['stage'] )? true : false;
		if( $isStage )
		{
			$field.= ", stg.status AS stage_status, stg.proceed_status, stg.halt_status ";
			$table.= $wpdb->prepare( 
				"LEFT JOIN {$dbname}{$this->tables['stage_header']} stg ON stg.ref_type = %s AND stg.ref_id = a.id ", 
				$this->section_id 
			);
		}

		//join warehouse
		$isWH = ( $args && $args['warehouse'] )? true : false;
		if( $isWH )
		{
			$field.= ", wh.name AS wh_name, wh.code AS wh_code, wh.indication AS wh_indication ";
			$table.= "LEFT JOIN {$dbname}{$this->tables['warehouse']} wh ON wh.comp_id = a.id ";
			...
			$isWhUse = ( $args && $args['wh_only'] )? true : false;
			if( $isWhUse )
			{
				$cond.= "AND wh.id IS NOT NULL ";
			}
		}

		//join supplier
		$isWH = ( $args && $args['supplier'] )? true : false;
		if( $isWH )
		{
			$field.= ", sup.name AS sup_name, sup.code AS sup_code ";
			$table.= "LEFT JOIN {$dbname}{$this->tables['supplier']} sup ON sup.comp_id = a.id ";

			if( isset( $filters['sup_code'] ) )
			{
				$cond.= $wpdb->prepare( "AND sup.code = %s ", $filters['sup_code'] );
			}

			$isSupUse = ( $args && $args['sup_only'] )? true : false;
			if( $isWhUse )
			{
				$cond.= "AND sup.id IS NOT NULL ";
			}
		}
		...
		if( isset( $filters['custno'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.custno = %s ", $filters['custno'] );
		}
		...
		if( isset( $filters['regno'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.regno = %s ", $filters['regno'] );
		}
		...
		{
			$cond.= $wpdb->prepare( "AND a.tin = %s ", $filters['tin'] );
		}
		if( isset( $filters['id_type'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.id_type = %s ", $filters['id_type'] );
		}
		if( isset( $filters['id_code'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.id_code = %s ", $filters['id_code'] );
		}
		if( isset( $filters['sst_no'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.sst_no = %s ", $filters['sst_no'] );
		}
		if( isset( $filters['einv'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.einv = %s ", $filters['einv'] );
		}
		...
		{
			$cond.= $wpdb->prepare( "AND ( a.created_by = %s OR a.lupdate_by = %s ) ", $filters['action_by'], $filters['action_by'] );
		}
		if( isset( $filters['s'] ) )
		{
            ...
            foreach( $search as $kw )
            {
				...
				if( $isParent )
				{
					$cd[] = "prt.custno LIKE '%".$kw."%' ";
					$cd[] = "prt.code LIKE '%".$kw."%' ";
					$cd[] = "prt.regno LIKE '%".$kw."%' ";
					$cd[] = "prt.name LIKE '%".$kw."%' ";
				}
	    ...
	    if( $args['meta'] )
		{
			foreach( $args['meta'] as $meta_key )
			{
				...
				$table.= $wpdb->prepare( "LEFT JOIN {$dbname}{$this->tables['meta']} {$meta_key} ON {$meta_key}.company_id = a.id AND {$meta_key}.meta_key = %s ", $meta_key );
	...
	public function check_is_under_same_company( $comp = '', $target_comp = '', $type = "code" )
	{
		if( ! $comp || ! $target_comp ) return false;

		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();

		$tbl1 = "{$this->tables['main']} s ";
		$tbl1.= "INNER JOIN {$this->tables['tree']} st ON st.ancestor = s.id ";
		$tbl1.= "LEFT JOIN {$this->tables['main']} a ON a.id = st.descendant ";
		$cond1 = "";

		$tbl2 = "{$this->tables['main']} s ";
		$tbl2.= "INNER JOIN {$this->tables['tree']} st ON st.descendant = s.id ";
		$tbl2.= "LEFT JOIN {$this->tables['main']} a ON a.id = st.ancestor ";
		$cond2 = "";
		...
		{
			case 'code':
				$cond1.= $wpdb->prepare( "AND s.code = %s ", $comp );
				$cond2.= $wpdb->prepare( "AND s.code = %s ", $comp );
				$cond.= $wpdb->prepare( "AND main.code = %s ", $target_comp );
			break;
			case 'id':
			default:
				$cond1.= $wpdb->prepare( "AND s.id = %s ", $comp );
				$cond2.= $wpdb->prepare( "AND s.id = %s ", $comp );
				$cond.= $wpdb->prepare( "AND main.id = %s ", $target_comp );
			break;
		}

		$sql1 = "SELECT a.* FROM {$tbl1} WHERE 1 {$cond1} ";
		$sql2 = "SELECT a.* FROM {$tbl2} WHERE 1 {$cond2} ";
		$union = $sql1." UNION ALL ".$sql2;

		$sql = "SELECT main.* FROM ( {$union} ) main WHERE 1 {$cond} ;";

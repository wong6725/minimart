...
if ( !class_exists( "WCWH_PurchasePricing" ) )
{

class WCWH_PurchasePricing extends WCWH_CRUD_Controller
{
    ...
    public $Notices;
    public $className = "PurchasePricing";
    ...
    public $price_type = 'purchase_price';
    ...
    public function get_latest_price( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
    {
        ...
        if( $isStk )
        {
            ...
            $field.= ", ROUND( stk.total_in_unit / stk.total_in, 3 ) AS average_in_unit ";
            $field.= ", IF( @needMetric, stk.total_in_unit / stk.total_in * pr.unit_price, 0 ) AS avg_unit_price ";
    ...
    public function get_price_list( $filters = [], $run = false )
    {
        ...
        //----------------------------------------
            ...
            $cd = $wpdb->prepare( "AND b.product_id = i.id AND a.type = %s ", $this->price_type );
    ...
    //---------------------------------------------------------------------------------------------------------
        ...
        $cond = $wpdb->prepare( "AND b.product_id = %s AND c.seller = %s AND a.type = %s ", $prdt_id, $seller, $this->price_type );

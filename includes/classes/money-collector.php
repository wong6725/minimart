...
if ( !class_exists( "WCWH_MoneyCollector_Class" ) ) 
{

class WCWH_MoneyCollector_Class extends WC_DocumentTemplate 
{
	...
	public $Notices;
	public $className = "MoneyCollector_Class";

	private $doc_type = 'money_collector';
	...
	// public function mc_action_handle( $action , $header = array() , $details  = array() )
	// {
	...
	// 				$header_item['sdocno'] = !empty( $header['sdocno'] )? $header['sdocno'] : $this->mc_generate_docno($sdocno,$this->doc_type,$doc_prefix);
	...
	// 				}
	// 				$doc_id = $this->mc_add_document_header( $header_item );
	// 				if( ! $doc_id )
	// 				{
	// 					$succ = false;
	...
	// 							$detail_id = $this->mc_add_document_items( $ditem );
	// 							if( ! $detail_id )
	// 								$succ = false;
	...
	// 				$exist_header = $this->mc_get_document_header( $header_item['doc_id'] ); 
	// 				if( ! $exist_header )
	// 				{
	...
	// 					$succ = $this->mc_update_document_header( array ( 'doc_id' => $header_item['doc_id'] ) , $upd_header );
	// 				}
	...
	// 					{
	// 						$exist_items_arr = $this->mc_get_exists_document_items_by_item_id( $arr_item_id );
	...
	// 							else 
	// 							{
	// 								$exist_item = $this->mc_get_exists_document_items( $ditem['doc_id'] , $ditem['product_id'] ,$ditem['ref_doc_id'] ,$ditem['ref_item_id'] , $ditem['block']);	
	// 							}
	// 							if( ! $exist_item )
	// 							{
	// 								$ditem['status'] = isset( $detail_item['dstatus'] )? $detail_item['dstatus'] : 1;
	...
	// 								if ( ! $this->mc_update_document_items( array ( 'item_id' => $exist_item['item_id']) , $upd_item ) )
	// 								{
	// 									$succ = false;
	// 								}
	...
	// 					{
	// 						$exists_delete_item = $this->mc_get_deletion_document_items( $header_item['doc_id'] , $active_item ); //Item to be deleted
	...
	// 							if( $succ )
	// 								$succ = $this->mc_delete_document_items( $header_item['doc_id'] , $active_item );
	// 						}
	// 					}
	// 				}
	// 			}
	...
	// 				return false;
	// 			}
	// 			$exists = $this->mc_get_document_header( $header_item['doc_id'], "1" );
	// 			if( ! $exists )
	// 			{
	// 				if( $this->Notices ) $this->Notices->set_notice( "invalid-record", "error", $this->_doc_type."|document_action_handle" );
	// 				return false;
	...
	// 				//Inactive Header
	// 				$succ = $this->mc_update_document_header( array( 'doc_id' => $header_item['doc_id'] , 'status' => $exists['status'] ) , $del_item ); //V1.0.3

	// 				//OFFSET uqty - V1.0.1
	// 				$exists_delete_item = $this->mc_get_deletion_document_items( $header_item['doc_id'] ); //Item to be deleted
	...
	// 					//Inactive Item
	// 					if( $succ )
	// 						$succ = $this->mc_update_document_items( array( 'doc_id' => $header_item['doc_id'] , 'status' => $exists['status'] ) , $del_item );//V1.0.3
	// 				}
	...
	// 				return false;
	// 			}
	// 			$exists = $this->mc_get_document_header( $header_item['doc_id'] , $this->_stat_to_post, "1" );
	// 			if( ! $exists )
	// 			{
	// 				if( $this->Notices ) $this->Notices->set_notice( "invalid-record", "error", $this->_doc_type."|document_action_handle" );
	// 				return false;
	...
	// 				//Post Header
	// 				$succ = $this->mc_update_document_header( array( 'doc_id' => $header_item['doc_id'], 'status' => $exists['status'] ) , $post_header );//V1.0.3
	// 				if( $succ && ! $this->_no_details )
	// 				{
	// 					//Post Item
	// 					$succ = $this->mc_update_document_items( array( 'doc_id' => $header_item['doc_id'], 'status' => $exists['status'] ) , $post_item );//V1.0.3
	// 				}

	// 			}
	// 			$this->header_item = $exists; // = $header; V1.0.4
	// 			$this->detail_item = $this->mc_get_document_items_by_doc( $this->header_item['doc_id'] );
	// 		break;
	...
	// 				return false;
	// 			}
	// 			$exists = $this->mc_get_document_header( $header_item['doc_id'] , $this->stat['post'] ); //Only Posted Status can be Unpost
	// 			if( ! $exists )
	// 			{
	// 				if( $this->Notices ) $this->Notices->set_notice( "invalid-record", "error", $this->_doc_type."|document_action_handle" );
	// 				return false;
	...
	// 				//Post Header
	// 				$succ = $this->mc_update_document_header( array( 'doc_id' => $header_item['doc_id'], 'status' => $exists['status'] ) , $post_header );
	// 				if( $succ && ! $this->_no_details )
	// 				{
	// 					//Post Item
	// 					$succ = $this->mc_update_document_items( array( 'doc_id' => $header_item['doc_id'], 'status' => $exists['status'] ) , $post_item );
	// 				}
	// 			}
	// 			$this->header_item = $exists; // = $header; V1.0.4
	// 			$this->detail_item = $this->mc_get_document_items_by_doc( $this->header_item['doc_id'] );
	...
	// 				return false;
	// 			}
	// 			$exists = $this->mc_get_document_header( $header_item['doc_id'] );
	// 			if( ! $exists )
	// 			{
	// 				if( $this->Notices ) $this->Notices->set_notice( "invalid-record", "error", $this->_doc_type."|document_action_handle" );
	// 				return false;
	...
	// 				//Inactive Header
	// 				$succ = $this->mc_update_document_header( array( 'doc_id' => $header_item['doc_id'] , 'status' => $exists['status'], 'flag' => $exists['flag'] ) , $act_item ); //V1.0.3
	// 			}
	// 			$this->header_item = $exists;
	// 			$this->detail_item = $this->mc_get_document_items_by_doc( $this->header_item['doc_id'] );
	// 		break;
	...
	// 				return false;
	// 			}
	// 			$exists = $this->mc_get_document_header( $header_item['doc_id'], "1", "0" );
	// 			if( ! $exists )
	// 			{
	// 				if( $this->Notices ) $this->Notices->set_notice( "invalid-record", "error", $this->_doc_type."|document_action_handle" );
	// 				return false;
	...
	public function mc_generate_docno( $sdocno, $doc_type = '', $def_prefix = '', $args = array() )
	{
		if( empty( $doc_type ) )
			return $sdocno;
	...
	public function mc_add_document_header( $item ){
		global $wpdb;
		$dbname= $this->dbName().$this->tables['document'];
		$this->mc_insert_replace_helper(
			$dbname,
			array(
				'docno' 			=> $item['docno'],
				'sdocno' 			=> $item['sdocno'],
	...
	public function mc_add_document_items( $item ){
		global $wpdb;
	...
	public function mc_get_document_header( $doc_id , $status = '', $flag = '' ){
		global $wpdb;
		$dbname = $this->dbName();
		if ( ! $doc_id ) {
			return false;
		}
		$get_items_sql  = $wpdb->prepare( "SELECT * FROM ".$this->tables['document']." WHERE doc_id = %d ", $doc_id );
	...
	public function mc_get_exists_document_items_by_item_id( $item_id_arr = array() ){
		global $wpdb;
		$dbname = $this->dbName();
	...
	public function mc_get_exists_document_items( $doc_id, $product_id , $ref_doc_id = 0, $ref_item_id = 0 , $block = 0 ){
		global $wpdb;
		$dbname = $this->dbName();
	...
	public function mc_get_document_items_by_doc( $doc_id , $status = 'active' ){
		global $wpdb;
		$dbname = $this->dbName();
		if ( ! $doc_id ) {
			return false;
	...
	public function mc_get_deletion_document_items( $doc_id , $active_items = array() ){
		global $wpdb;
		$dbname = $this->dbName();
		if ( ! $doc_id  ) {
			return false;
	...
	public function mc_update_document_header( $cond , $item ){
		global $wpdb;
		$dbname= $this->dbName();
		if ( ! $cond || ! $item) {
			return false;
		}
		$update = $this->mc_update( $dbname.$this->tables['document'], $item, $cond );
	...
	public function mc_update_document_items( $cond , $item ){
		global $wpdb;
		$dbname = $this->dbName();
		if ( ! $cond || ! $item ) {
			return false;
		}
		$update = $this->mc_update( $dbname.$this->tables['document_items'], $item, $cond );
	...
	public function mc_meta_handle( $doc_id, $header_item, $sanitize = false )
	{
		...
		foreach( $header_item as $key => $value )
		{
			if( !in_array( $key, $header_keys ) )
			{
				...
				if( ! $this->mc_add_document_meta_value( $key , $value , $doc_id ) )
				{
					$succ = false;
					break;
				}
	...
	public function mc_add_document_meta_value( $meta_key , $meta_value , $doc_id, $item_id = 0 ){
		global $wpdb;
		$succ = true;
		...
		$exists = $this->mc_get_doc_meta( $doc_id , $meta_key , $item_id );
		if ( ! $exists )
		{
			...
			{
				if( ! $this->mc_delete_document_meta( array( "meta_key" => $meta_key , "doc_id" => $doc_id , "item_id" => $item_id ) ) ) //V1.0.3
					return false;
			}
			else //UPDATE IF NOT EMPTY
			{
				$meta_id = $exists['meta_id']; //V1.0.3
				if( ! $this->mc_update_document_meta( $exists['meta_id'] , array( "meta_value" => $meta_value) ) )
				{
					$succ = false;
				}
			}
	...
	public function mc_get_doc_meta( $doc_id , $meta_key, $item_id = 0, $val_only = false ){
		global $wpdb;
		$dbname = $this->dbName();
	...
	public function mc_delete_document_items( $doc_id , $active_items = array() , $excluded = true ){
		global $wpdb;
		$dbname = $this->dbName();
		if ( ! $doc_id ) {
			return false;
	...
	public function mc_delete_document_meta( $cond ){
		global $wpdb;
		$dbname = $this->dbName();
		if ( ! $cond ) {
			return false;
		}
		$update = $this->mc_delete( $dbname.$this->tables['document_meta'], $cond );
	...
	*/
	...
	/*
	public function mc_update_document_meta( $meta_id , $item ){
		global $wpdb;
		$dbname = $this->dbName();
		if ( ! $meta_id ) {
			return false;
		}
		$update = $this->mc_update( $dbname.$this->tables['document_meta'], $item, array( 'meta_id' => $meta_id ) );
	...
	public function mc_delete( $table, $where, $where_format = null ) {
		global $wpdb;
		if ( ! is_array( $where ) ) {
			return false;
		}
	...
	public function add_document_meta( $doc_id = 0, $meta_key = '', $meta_value = '', $item_id = 0 )
	{
		...
		global $wpdb;
		$dbname = $this->dbName();
		$succ=$this->mc_insert_replace_helper(
			$dbname.$this->tables['document_meta'],
			array( 
				'doc_id'	=> $doc_id, 
				'item_id'	=> $item_id,
	...
	public function mc_add_document_meta( $item ){
		global $wpdb;
		$dbname = $this->dbName();
		$this->mc_insert_replace_helper(
			$dbname.$this->tables['document_meta'],
	...
	public function mc_detail_meta_handle( $doc_id, $detail_item, $sanitize = false ){
			...
			if( $items ){
				foreach( $items as $key => $value ){
					...
					if( !in_array( $key, $detail_keys ) ){
						if( ! $this->mc_add_document_meta_value( $key , $value , $doc_id, $items['item_id'] ) )
						{
							$succ = false;
							break;
						}
	...
	public function mc_insert_replace_helper( $table, $data, $format = null, $type = 'INSERT' ) {
		global $wpdb;
		$wpdb->insert_id = 0;
	...
	public function mc_update( $table, $data, $where, $format = null, $where_format = null ) {
		global $wpdb;

...
if ( !class_exists( "WCWH_Addresses_Class" ) ) 
{

class WCWH_Addresses_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_addresses";

	protected $tbl = "addresses";
	...
	public $Notices;
	public $className = "Addresses_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( isset( $filters['addr_type'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.addr_type = %s ", $filters['addr_type'] );
		}
		if( isset( $filters['country'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.country = %s ", $filters['country'] );
		}
		if( isset( $filters['state'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.state = %sd ", $filters['state'] );
		}
		if( isset( $filters['city'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.city = %s ", $filters['city'] );
		}
		if( isset( $filters['postcode'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.postcode = %s ", $filters['postcode'] );
		}
		if( isset( $filters['s'] ) )
		{
            ...
            foreach( $search as $kw )
            {
                $kw = trim( $kw );
                $cd = array();
                $cd[] = "a.address_1 LIKE '%".$kw."%' ";
				$cd[] = "a.address_2 LIKE '%".$kw."%' ";
				$cd[] = "a.contact_person LIKE '%".$kw."%' ";
				$cd[] = "a.contact_no LIKE '%".$kw."%' ";

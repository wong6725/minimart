...
if ( !class_exists( "WCWH_Asset_Class" ) ) 
{

class WCWH_Asset_Class extends WCWH_CRUD_Controller 
{
	...
	public function set_db_tables()
	{
		...
		$this->tables = array(
			...
			"asset_movement"	=> $prefix."asset_movement",
	...
	public function update_metas( $id, $metas )
	{
		...
		foreach( $metas as $key => $value )
		{
			if( is_array( $value ) )
			{
				delete_asset_meta( $id, $key );
				foreach( $value as $val )
				{
					add_asset_meta( $id, $key, $val );
				}
			}
			else
			{
				update_asset_meta( $id, $key, $value );
			}
		}
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isParent )
		{
			$field.= ", prt.name AS prt_name, prt.code AS prt_code, prt.serial AS prt_serial, prt.type AS prt_type ";
	    ...
	    if( $args['meta'] )
		{
			foreach( $args['meta'] as $meta_key )
			{
				...
				$table.= $wpdb->prepare( "LEFT JOIN {$dbname}{$this->tables['meta']} {$meta_key} ON {$meta_key}.asset_id = a.id AND {$meta_key}.meta_key = %s ", $meta_key );

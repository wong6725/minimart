...
if ( !class_exists( "WCWH_CreditTerm_Class" ) ) 
{

class WCWH_CreditTerm_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_credit_term";

	protected $tbl = "credit_term";
	...
	public $Notices;
	public $className = "CreditTerm_Class";
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( $isParent )
		{
			$field.= ", prt.name AS prt_name, prt.days AS prt_days, prt.offset AS prt_offset, prt.apply_date AS prt_apply_date ";
			$table.= "LEFT JOIN {$dbname}{$this->tables['main']} prt ON prt.id = a.parent ";
		}

		$isPeriod = ( $args && $args['period'] )? true : false;
		if( $isPeriod )
		{
			$field.= ", @today:= DATE_FORMAT( 
			IF( a.apply_date IS NOT NULL AND a.apply_date != '', CONCAT( a.apply_date, '-15' ), NOW() ), '%Y-%m-%d' ) AS today 
			, CASE 
			WHEN DAY(@today) >= a.days
				THEN CONCAT( DATE_FORMAT( @today, '%Y-%m-' ), a.days ) + INTERVAL a.offset DAY 
			WHEN DAY(@today) < a.days
				THEN CONCAT( DATE_FORMAT( @today - INTERVAL 1 MONTH, '%Y-%m-' ), a.days ) + INTERVAL a.offset DAY 
			END AS from_date 
			, CASE WHEN DAY(@today) >= a.days
				THEN CONCAT( DATE_FORMAT( @today + INTERVAL 1 MONTH, '%Y-%m-' ), a.days ) - INTERVAL 1 DAY 
			WHEN DAY(@today) < a.days
				THEN CONCAT( DATE_FORMAT( @today, '%Y-%m-' ), a.days ) - INTERVAL 1 DAY 
			END AS to_date ";
		}
		...
		{
			$cond.= $wpdb->prepare( "AND a.days = %s ", $filters['days'] );
		}
		if( isset( $filters['offset'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.offset = %s ", $filters['offset'] );
		}
		...
		{
			$cond.= $wpdb->prepare( "AND a.parent = %s ", 0 );
		}
		if( isset( $filters['apply_date'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.apply_date = %s ", $filters['apply_date'] );
		}
		if( isset( $filters['s'] ) )
		{

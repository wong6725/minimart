<?php
//Steven create UOM conversion class based on UOM class
if ( !defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

if ( !class_exists( "WCWH_UOM_Class" ) ) 
{

class WCWH_UOMConversion_Class extends WCWH_CRUD_Controller 
{
	protected $section_id = "wh_uom_conversion";

	protected $tbl = "uom_conversion";
	...
	public $Notices;
	public $className = "UOMConversion_Class";
		...
		if( $succ )
		{
			...
			switch ( $action )
			{
				...
				case "update":
					...
					$datas['from_uom'] = strtoupper( $datas['from_uom'] );
					$datas['to_uom'] = strtoupper( $datas['to_uom'] );
	...
	public function get_infos( $filters = [], $order = [], $single = false, $args = [], $group = [], $limit = [] )
	{
		...
		if( isset( $filters['from_uom'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.from_uom = %s ", $filters['from_uom'] );
		}
		if( isset( $filters['to_uom'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.to_uom = %s ", $filters['to_uom'] );
		}
		if( isset( $filters['from_unit'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.from_unit = %s ", $filters['from_unit'] );
		}
		if( isset( $filters['to_unit'] ) )
		{
			$cond.= $wpdb->prepare( "AND a.to_unit = %s ", $filters['to_unit'] );
		}
		if( isset( $filters['s'] ) )
		{

...
if( ! class_exists( 'WCWH_AccountType_Class' ) ) include_once( WCWH_DIR . "/includes/classes/account-type.php" ); 

if ( !class_exists( "WCWH_AccountType_Controller" ) ) 
{

class WCWH_AccountType_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "AccountType_Controller";
	...
	{
		$this->Logic = new WCWH_AccountType_Class( $this->db_wpdb );
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( $datas[ $this->get_primaryKey() ] && $action == 'update' )
					{
						...
						if( ! $datas['auto_topup'] ) $datas['auto_topup'] = 0;
					}
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Account Type" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Account Type"
				>
					<?php echo $actions['save'] ?> Account Type
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/accountType-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/accountType-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/accountTypeListing.php" ); 
			$Inst = new WCWH_AccountType_Listing();
			...
			$datas = $this->Logic->get_infos( $filters, $order, false, [ 'group'=>1, 'term'=>1 ] );

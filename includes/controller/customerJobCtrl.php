...
if ( !class_exists( "WCWH_CustomerJob_Class" ) ) include_once( WCWH_DIR . "/includes/classes/customer-job.php" ); 

if ( !class_exists( "WCWH_CustomerJob_Controller" ) ) 
{
	
class WCWH_CustomerJob_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_customer_job";

	protected $primary_key = "id";

	public $Notices;
	public $className = "CustomerJob_Controller";
	...
	{
		$this->Logic = new WCWH_CustomerJob_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Job / Position" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Job / Position"
				>
					<?php echo $actions['save'] ?> Job / Position
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/customerJob-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/customerJob-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/customerJobListing.php" ); 
			$Inst = new WCWH_CustomerJob_Listing();

...
if( ! class_exists( 'WCWH_UOM_Class' ) ) include_once( WCWH_DIR . "/includes/classes/uom.php" ); 

if ( !class_exists( "WCWH_UOM_Controller" ) ) 
{

class WCWH_UOM_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_uom";

	protected $primary_key = "id";

	public $Notices;
	public $className = "UOM_Controller";
	...
	{
		$this->Logic = new WCWH_UOM_Class( $this->db_wpdb );
	...
	public function im_ex_default_column( $params = array() )
	{
		...
		$default_column['title'] = [ 'ID', 'Code', 'Name','Fraction', 'Action' ];

		$default_column['default'] = [ 'id', 'code', 'name','fraction', 'action' ];
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> UOM" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> UOM"
				>
					<?php echo $actions['save'] ?> UOM
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/uom-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/uom-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/uomListing.php" ); 
			$Inst = new WCWH_UOM_Listing();

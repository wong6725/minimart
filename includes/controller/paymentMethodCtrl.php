...
if ( !class_exists( "WCWH_PaymentMethod_Class" ) ) include_once( WCWH_DIR . "/includes/classes/payment-method.php" ); 

if ( !class_exists( "WCWH_PaymentMethod_Controller" ) ) 
{
	
class WCWH_PaymentMethod_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "PaymentMethod_Controller";
	...
	{
		$this->Logic = new WCWH_PaymentMethod_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Payment Method" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Payment Method"
				>
					<?php echo $actions['save'] ?> Payment Method
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/paymentMethod-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/paymentMethod-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/paymentMethodListing.php" ); 
			$Inst = new WCWH_PaymentMethod_Listing();

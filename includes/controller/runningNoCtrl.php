...
if ( !class_exists( "WCWH_RunningNo_Class" ) ) include_once( WCWH_DIR . "/includes/classes/running-no.php" ); 

if ( !class_exists( "WCWH_RunningNo_Controller" ) ) 
{
	
class WCWH_RunningNo_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "RunningNo_Controller";
	...
	{
		$this->Logic = new WCWH_RunningNo_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Running No" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Running No"
				>
					<?php echo $actions['save'] ?> Running No
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/runningNo-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/runningNo-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/runningNoListing.php" ); 
			$Inst = new WCWH_RunningNo_Listing();

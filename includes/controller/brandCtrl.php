...
if ( !class_exists( "WCWH_Brand_Class" ) ) include_once( WCWH_DIR . "/includes/classes/brand.php" ); 

if ( !class_exists( "WCWH_Brand_Controller" ) ) 
{

class WCWH_Brand_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_brand";

	protected $primary_key = "id";

	public $Notices;
	public $className = "Brand_Controller";
	...
	{
		$this->Logic = new WCWH_Brand_Class( $this->db_wpdb );
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $datas['code'] )
					{
						if( $datas[ $this->get_primaryKey() ] )
						{
							$scode = get_brand_meta( $datas[ $this->get_primaryKey() ], 'scode', true );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Brand" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Brand"
				>
					<?php echo $actions['save'] ?> Brand
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/brand-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/brand-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/brandListing.php" ); 
			$Inst = new WCWH_Brand_Listing();

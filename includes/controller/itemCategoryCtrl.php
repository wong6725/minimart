...
if ( !class_exists( "WCWH_ItemCategory_Class" ) ) include_once( WCWH_DIR . "/includes/classes/item-category.php" ); 

if ( !class_exists( "WCWH_ItemCategory_Controller" ) ) 
{
	
class WCWH_ItemCategory_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "ItemCategory_Controller";
	...
	{
		$this->Logic = new WCWH_ItemCategory_Class( $this->db_wpdb );
	...
	public function action_handler( $action, $datas = array(), $obj = array(), $transact = true )
	{
		...
		try
        {
			...
			if( !empty( $datas['id'] ) )
			{
				$datas[ $this->get_primaryKey() ] = $datas['id'];
				unset( $datas['id'] );
			}
        	...
        	switch ( $action )
        	{
				...
				case "delete":
					...
					$ids = $datas[ $this->get_primaryKey() ];
					$ids = is_array( $datas[ $this->get_primaryKey() ] )? $datas[ $this->get_primaryKey() ] : array( $datas[ $this->get_primaryKey() ] );
	...
	public function after_action( $succ, $id, $action = "save", $handled = [] )
	{
		...
		if( $succ )
		{
			...
			foreach( $id as $ref_id )
			{
				...
				$api_data = [];
				$api_data[] = [
					'id' => $handled[ $ref_id ]['term_id'],
					'name' => $handled[ $ref_id ]['name'],
					'slug' => $handled[ $ref_id ]['slug'],
					'parent' => $handled[ $ref_id ]['parent'],
					'description' => $handled[ $ref_id ]['description'],
					'action' => $action,
				];
				$succ = apply_filters( 'wcwh_sync_arrangement', $ref_id, $this->section_id, $action, $handled[ $ref_id ]['slug'], '', $api_data );
				if( ! $succ )
				{
					$this->Notices->set_notice( 'arrange-fail', 'error' );
				}
	...
	public function im_ex_default_column( $params = array() )
	{
		...
		$default_column['title'] = [ 'ID', 'Name' ,'Code', 'Parent','Description', 'Action' ];

		$default_column['default'] = [ 'id', 'name' ,'slug', 'parent', 'description', 'action' ];

		$default_column['unique'] = array( 'slug' );

		$default_column['required'] = array( 'slug', 'name' );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Category" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Category"
				>
					<?php echo $actions['save'] ?> Category
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/itemCategory-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/itemCategory-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/itemCategoryListing.php" ); 
			$Inst = new WCWH_ItemCategory_Listing();
			...
			$datas = $this->Logic->get_infos( $filters, $order, false, [ 'parent'=>true, 'tree'=>1 ] );

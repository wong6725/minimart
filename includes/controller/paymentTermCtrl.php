...
if ( !class_exists( "WCWH_PaymentTerm_Class" ) ) include_once( WCWH_DIR . "/includes/classes/payment-term.php" ); 

if ( !class_exists( "WCWH_PaymentTerm_Controller" ) ) 
{
	
class WCWH_PaymentTerm_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_payment_term";

	protected $primary_key = "id";

	public $Notices;
	public $className = "PaymentTerm_Controller";
	...
	{
		$this->Logic = new WCWH_PaymentTerm_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Payment Term" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Payment Term"
				>
					<?php echo $actions['save'] ?> Payment Term
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/paymentTerm-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/paymentTerm-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/paymentTermListing.php" ); 
			$Inst = new WCWH_PaymentTerm_Listing();

...
if( ! class_exists( 'WCWH_TodoAction_Class' ) ) include_once( WCWH_DIR . "/includes/classes/todo-action.php" ); 

if( ! class_exists( 'WCWH_Permission_Controller' ) ) include_once( WCWH_DIR . "/includes/controller/permissionCtrl.php" ); 

if ( !class_exists( "WCWH_TodoAction_Controller" ) ) 
{

class WCWH_TodoAction_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "TodoAction_Controller";
	...
	{
		$this->Logic = new WCWH_TodoAction_Class( $this->db_wpdb );
		$this->Logic->set_section_id( $this->section_id );

		$this->Permission = new WCWH_Permission_Controller();
	}
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Todo Action" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Todo Action"
				>
					<?php echo $actions['save'] ?> Todo Action
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		$permissions = $this->Permission->get_permission_info( true );
		if( $permissions )
		{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/todoAction-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/todoAction-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/todoActionListing.php" ); 
			$Inst = new WCWH_TodoAction_Listing();

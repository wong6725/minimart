...
if( ! class_exists( 'WCWH_Status_Class' ) ) include_once( WCWH_DIR . "/includes/classes/status.php" ); 

if ( !class_exists( "WCWH_Status_Controller" ) ) 
{

class WCWH_Status_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Status_Controller";
	...
	{
		$this->Logic = new WCWH_Status_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Status" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Status"
				>
					<?php echo $actions['save'] ?> Status
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/status-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/status-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/statusListing.php" ); 
			$Inst = new WCWH_Status_Listing();

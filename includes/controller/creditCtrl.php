...
if ( !class_exists( "WCWH_Credit_Class" ) ) include_once( WCWH_DIR . "/includes/classes/credit.php" ); 

if ( !class_exists( "WCWH_Credit_Controller" ) ) 
{
	
class WCWH_Credit_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Credit_Controller";

	public $Logic;

	public $tplName = array(
		'newGroup' 		=> 'newGroupCredit',
		'newCustomer'	=> 'newCustomerCredit',
	);

	public $useFlag = false;

	protected $section = 'credit_limit';
	...
	{
		$this->Logic = new WCWH_Credit_Class( $this->db_wpdb );
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $datas[ $this->get_primaryKey() ] && $action == 'save' )
					{
						...
						$scheme = get_scheme( $this->section, ( $datas['scheme'] )? $datas['scheme'] : 'default' );
	...
	public function view_fragment( $type = 'save' )
	{
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['newGroup'] ?>" 
					data-title="<?php echo $actions['save'] ?> Credit" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Credit"
				>
					<?php echo $actions['save'] ?> Credit by Group
			...
			case 'save-customer':
				if( current_user_cans( [ 'save_'.$this->section_id ] ) && ! $this->view_outlet ):
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['newCustomer'] ?>" 
					data-title="<?php echo $actions['save'] ?> Credit" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Credit"
				>
					<?php echo $actions['save'] ?> Credit by Customer
					<i class="fa fa-plus-circle" aria-hidden="true"></i>
				</button>
			<?php
				endif;
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $id )
		{
			...
			if( $datas )
			{
				...
				$args['scheme'] = $datas['scheme'];
				switch( $args['scheme'] )
				{
					case 'customer_group':
						$args['tplName'] = $this->tplName['newGroup'];
					break;
					case 'customer':
						$args['tplName'] = $this->tplName['newCustomer'];
					break;
				}
		...
		do_action( 'wcwh_get_template', 'form/credit-form.php', $args );
	}

	public function view_customer_form( $id = 0, $templating = true, $isView = false )
	{
		$args = array(
			'setting'	=> $this->setting,
			'section'	=> $this->section_id,
			...
			'token'		=> apply_filters( 'wcwh_generate_token', $this->section_id ),
			'new'		=> 'new',
			'tplName'	=> $this->tplName['newCustomer'],
			'scheme'	=> 'customer',
		);
		if( $this->warehouse['id'] && $this->view_outlet ) $args['seller'] = $this->warehouse['id'];
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/credit-form.php', $this->tplName['newCustomer'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/credit-form.php', $args );
	...
	public function view_group_form( $id = 0, $templating = true, $isView = false )
	{
		$args = array(
			'setting'	=> $this->setting,
			'section'	=> $this->section_id,
			...
			'token'		=> apply_filters( 'wcwh_generate_token', $this->section_id ),
			'new'		=> 'new',
			'tplName'	=> $this->tplName['newGroup'],
			'scheme'	=> 'customer_group',
		);
		if( $this->warehouse['id'] && $this->view_outlet ) $args['seller'] = $this->warehouse['id'];
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/credit-form.php', $this->tplName['newGroup'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/credit-form.php', $args );
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/creditListing.php" ); 
			$Inst = new WCWH_Credit_Listing();

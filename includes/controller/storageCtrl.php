...
if ( !class_exists( "WCWH_Storage_Class" ) ) include_once( WCWH_DIR . "/includes/classes/storage.php" ); 

if ( !class_exists( "WCWH_Storage_Controller" ) ) 
{

class WCWH_Storage_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Storage_Controller";
	...
	{
		$this->Logic = new WCWH_Storage_Class( $this->db_wpdb );
	...
	protected function generate_serial()
	{
		...
		do 
		{
			$serial = apply_filters( 'warehouse_generate_docno', '', 'storage_serial' );
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $datas['code'] )
					{
						if( $datas[ $this->get_primaryKey() ] )
						{
							$scode = get_storage_meta( $datas[ $this->get_primaryKey() ], 'scode', true );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Storage" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Storage"
				>
					<?php echo $actions['save'] ?> Storage
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $this->warehouse )
		{
			$args['data']['wh_code'] = $this->warehouse['code'];
		}
		...
		if( $id )
		{
			...
			$datas = $this->Logic->get_infos( $filters, [], true, [ 'parent'=>1 ] );
			if( $datas )
			{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/storage-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/storage-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/storageListing.php" ); 
			$Inst = new WCWH_Storage_Listing();
			...
			if( !current_user_cans( [ 'wh_support' ] ) ) $hid = true;
			$count = $this->Logic->count_statuses( $this->warehouse['code'], $hid );
			if( $count ) $Inst->viewStats = $count;

			if( empty( $filters['wh_code'] ) )
			{
				$filters['wh_code'] = $this->warehouse['code'];
			}

			$args = [ 'parent'=>1, 'tree'=>1 ];
			if( !current_user_cans( [ 'wh_support' ] ) ) $args['usable'] = 1;

			$order = $Inst->get_data_ordering();
			$limit = $Inst->get_data_limit();

			$datas = $this->Logic->get_infos( $filters, $order, false, $args, [], $limit );

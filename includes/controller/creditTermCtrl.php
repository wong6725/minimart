...
if ( !class_exists( "WCWH_CreditTerm_Class" ) ) include_once( WCWH_DIR . "/includes/classes/credit-term.php" ); 

if ( !class_exists( "WCWH_CreditTerm_Controller" ) ) 
{
	
class WCWH_CreditTerm_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "CreditTerm_Controller";
	...
	{
		$this->Logic = new WCWH_CreditTerm_Class( $this->db_wpdb );
	...
	public function validate( $action , $datas = array(), $obj = array() )
	{
		...
		if( $succ )
		{
			...
			if( $action == 'delete' && isset( $datas['id'] ) && $datas['id'] == 1 )
			{
				$succ = false;
				$this->Notices->set_notice( 'DEFAULT term does not allow to delete!', 'warning' );
			}
		}
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Term" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Credit Term"
				>
					<?php echo $actions['save'] ?> Credit Term
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/creditTerm-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/creditTerm-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/creditTermListing.php" ); 
			$Inst = new WCWH_CreditTerm_Listing();
			...
			$datas = $this->Logic->get_infos( $filters, $order, false, [ 'parent'=>true, 'period'=>true ], [], $limit );

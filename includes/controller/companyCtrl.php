...
if ( !class_exists( "WCWH_Company_Controller" ) ) 
{

class WCWH_Company_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Company_Controller";
	...
	{
		$this->Logic = new WCWH_Company_Class( $this->db_wpdb );
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $datas['code'] )
					{
						if( $datas[ $this->get_primaryKey() ] )
						{
							$scode = get_company_meta( $datas[ $this->get_primaryKey() ], 'scode', true );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Company" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Company"
				>
					<?php echo $actions['save'] ?> Company
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/company-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/company-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/companyListing.php" ); 
			$Inst = new WCWH_Company_Listing();

...
if( ! class_exists( 'WCWH_Section_Class' ) ) include_once( WCWH_DIR . "/includes/classes/section.php" ); 

if ( !class_exists( "WCWH_Section_Controller" ) ) 
{

class WCWH_Section_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Section_Controller";
	...
	{
		$this->Logic = new WCWH_Section_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Section" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Section"
				>
					<?php echo $actions['save'] ?> Section
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/section-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/section-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/sectionListing.php" ); 
			$Inst = new WCWH_Section_Listing();

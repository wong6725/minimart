...
if ( !class_exists( "WCWH_OriginGroup_Class" ) ) include_once( WCWH_DIR . "/includes/classes/origin-group.php" ); 

if ( !class_exists( "WCWH_OriginGroup_Controller" ) ) 
{
	
class WCWH_OriginGroup_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_origin_group";

	protected $primary_key = "id";

	public $Notices;
	public $className = "OriginGroup_Controller";
	...
	{
		$this->Logic = new WCWH_OriginGroup_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Customer Origin" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Customer Origin"
				>
					<?php echo $actions['save'] ?> Customer Origin
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/originGroup-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/originGroup-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/originGroupListing.php" ); 
			$Inst = new WCWH_OriginGroup_Listing();

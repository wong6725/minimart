...
if ( !class_exists( "WCWH_Template_Class" ) ) include_once( WCWH_DIR . "/includes/classes/template.php" ); 

if ( !class_exists( "WCWH_Template_Controller" ) ) 
{
	
class WCWH_Template_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Template_Controller";
	...
	{
		$this->Logic = new WCWH_Template_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?>" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?>"
				>
					<?php echo $actions['save'] ?>
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/template-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/template-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/templateListing.php" ); 
			$Inst = new WCWH_Template_Listing();

...
if ( !class_exists( "WCWH_Criteria_Class" ) ) include_once( WCWH_DIR . "/includes/classes/criteria.php" ); 

if ( !class_exists( "WCWH_Criteria_Controller" ) ) 
{

class WCWH_Criteria_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_criteria";

	protected $primary_key = "id";

	public $Notices;
	public $className = "Criteria_Controller";
	...
	{
		$this->Logic = new WCWH_Criteria_Class( $this->db_wpdb );
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $datas['code'] )
					{
						if( $datas[ $this->get_primaryKey() ] )
						{
							$scode = get_criteria_meta( $datas[ $this->get_primaryKey() ], 'scode', true );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Criteria" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Criteria"
				>
					<?php echo $actions['save'] ?> Criteria
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/criteria-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/criteria-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/criteriaListing.php" ); 
			$Inst = new WCWH_Criteria_Listing();

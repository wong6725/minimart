...
if ( !class_exists( "WCWH_VendingMachine_Class" ) ) include_once( WCWH_DIR . "/includes/classes/vending-machine.php" ); 

if ( !class_exists( "WCWH_VendingMachine_Controller" ) ) 
{

class WCWH_VendingMachine_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_vending_machine";

	protected $primary_key = "id";

	public $Notices;
	public $className = "VendingMachine_Controller";

	public $Logic;

	public $tplName = array(
		'new' => 'newVendingMachine',
	);
	...
	{
		$this->Logic = new WCWH_VendingMachine_Class( $this->db_wpdb );
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $datas['code'] )
					{
						if( $datas[ $this->get_primaryKey() ] )
						{
							$scode = get_vending_machine_meta( $datas[ $this->get_primaryKey() ], 'scode', true );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Machine" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Machine"
				>
					<?php echo $actions['save'] ?> Machine
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $id )
		{
			...
			$datas = $this->Logic->get_infos( $filters, [], true, [ 'company'=>1, 'warehouse'=>1 ] );
			if( $datas )
			{
				$metas = get_vending_machine_meta( $id );
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/vendingMachine-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/vendingMachine-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/vendingMachineListing.php" ); 
			$Inst = new WCWH_VendingMachine_Listing();
			...
			$metas = [ 'location' ];
			$datas = $this->Logic->get_infos( $filters, $order, false, [ 'company'=>1, 'warehouse'=>1, 'meta'=>$metas ], [], $limit );

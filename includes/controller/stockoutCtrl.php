...
if( ! class_exists( 'WCWH_Stockout_Class' ) ) include_once( WCWH_DIR . "/includes/classes/stockout.php" ); 

if ( !class_exists( "WCWH_Stockout_Controller" ) ) 
{

class WCWH_Stockout_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Stockout_Controller";
	...
	{
		$this->Logic = new WCWH_Stockout_Class( $this->db_wpdb );
	...
	public function view_fragment( $type = 'save' )
	{
		...
		switch( strtolower( $type ) )
		{
			case 'save':
			default:
				if( current_user_cans( [ 'currently_not_use' ] ) ):
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Stockout" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Stockout"
				>
					<?php echo $actions['save'] ?> Stockout
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/stockout-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/stockout-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/stockoutListing.php" ); 
			$Inst = new WCWH_Stockout_Listing();

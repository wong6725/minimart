...
if( ! class_exists( 'WCWH_ItemGroup_Class' ) ) include_once( WCWH_DIR . "/includes/classes/item-group.php" ); 

if ( !class_exists( "WCWH_ItemGroup_Controller" ) ) 
{

class WCWH_ItemGroup_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "ItemGroup_Controller";
	...
	{
		$this->Logic = new WCWH_ItemGroup_Class( $this->db_wpdb );
	...
	public function validate_unique( $action, $datas = array() )
	{
		...
		if( $unique )
		{
			...
			{
				$result = $this->Logic->get_infos( [ $key => strtoupper( $datas[$key] ) ], [], true );
				if( $result ) 
				{	
		...
		try
        {
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $datas['prefix'] ) $datas['prefix'] = $datas['code'];
	...
	public function im_ex_default_column( $params = array() )
	{
		...
		$default_column['title'] = [ 'ID', 'Code' ,'Prefix', 'Name','Desc', 'Action' ];

		$default_column['default'] = [ 'id', 'code' ,'prefix', 'name','desc', 'action' ];

		$default_column['unique'] = array( 'code' );

		$default_column['required'] = array( 'code', 'name' );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Item Group" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Item Group"
				>
					<?php echo $actions['save'] ?> Item Group
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/itemGroup-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/itemGroup-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/itemGroupListing.php" ); 
			$Inst = new WCWH_ItemGroup_Listing();

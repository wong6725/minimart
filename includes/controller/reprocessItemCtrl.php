...
if ( !class_exists( "WCWH_ReprocessItem_Class" ) ) include_once( WCWH_DIR . "/includes/classes/reprocess-item.php" ); 

if ( !class_exists( "WCWH_ReprocessItem_Controller" ) ) 
{

class WCWH_ReprocessItem_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "ReprocessItem_Controller";

	public $Logic;

	public $tplName = array(
		'new' => 'newReprocessItem',
	);
	...
	{
		$this->Logic = new WCWH_ReprocessItem_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Reprocess Item" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Reprocess Item"
				>
					<?php echo $actions['save'] ?> Reprocess Item
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/reprocessItem-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/reprocessItem-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/reprocessItemListing.php" ); 
			$Inst = new WCWH_ReprocessItem_Listing();

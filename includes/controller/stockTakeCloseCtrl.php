...
if ( !class_exists( "WCWH_StockTakeClose_Class" ) ) include_once( WCWH_DIR . "/includes/classes/stocktake-close.php" ); 

if ( !class_exists( "WCWH_StockTakeClose_Controller" ) ) 
{

class WCWH_StockTakeClose_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_stocktake_close";

	public $Notices;
	public $className = "StockTakeClose_Controller";

	public $Logic;

	public $tplName = array(
		'new' => 'newStockTakeClose',
		'row' => 'rowStockTakeClose',
	);
	...
	{
		$this->Logic = new WCWH_StockTakeClose_Class( $this->db_wpdb );
		...
		try
        {
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					$header['doc_date'] = ( $header['doc_date'] )? date( 'Y-m-d 23:59:59', strtotime( $header['doc_date'] ) ) : current_time( 'Y-m-d 23:59:59' );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="Stock Take Control" data-modal="wcwhModalForm" 
	...
	public function view_form( $id = 0, $templating = true, $isView = false, $getContent = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/stockTakeClose-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/stockTakeClose-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/stockTakeCloseListing.php" ); 
			$Inst = new WCWH_StockTakeClose_Listing();

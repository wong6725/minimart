...
if ( !class_exists( "WCWH_TodoArrangement_Class" ) ) include_once( WCWH_DIR . "/includes/classes/todo-arrangement.php" ); 

if ( !class_exists( "WCWH_TodoArrangement_Controller" ) ) 
{
	
class WCWH_TodoArrangement_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "TodoArrangement_Controller";
	...
	{
		$this->Logic = new WCWH_TodoArrangement_Class( $this->db_wpdb );
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Todo Arrangement" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Todo Arrangement"
				>
					<?php echo $actions['save'] ?> Todo Arrangement
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		$args = array(
			...
			'action_type' => $this->refs['action_type']
		);
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/todoArrangement-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/todoArrangement-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/todoArrangementListing.php" ); 
			$Inst = new WCWH_todoArrangement_Listing();

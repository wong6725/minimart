...
if( ! class_exists( 'WCWH_Scheme_Class' ) ) include_once( WCWH_DIR . "/includes/classes/scheme.php" ); 

if ( !class_exists( "WCWH_Scheme_Controller" ) ) 
{

class WCWH_Scheme_Controller extends WCWH_CRUD_Controller
{	
	...
	public $Notices;
	public $className = "Scheme_Controller";
	...
	{
		$this->Logic = new WCWH_Scheme_Class( $this->db_wpdb );
		...
		try
        {
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( $datas['scheme_lvl'] == 0 || ! $datas['scheme'] )
						$datas['scheme'] = 'default';
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Scheme" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Scheme"
				>
					<?php echo $actions['save'] ?> Scheme
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/scheme-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/scheme-form.php', $args );
		}
	}
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/listing/schemeListing.php" ); 
			$Inst = new WCWH_Scheme_Listing();

...
if ( !class_exists( "WCWH_Adjustment_Class" ) ) include_once( WCWH_DIR . "/includes/classes/adjustment.php" ); 

if ( !class_exists( "WCWH_Adjustment_Controller" ) ) 
{

class WCWH_Adjustment_Controller extends WCWH_CRUD_Controller
{	
	protected $section_id = "wh_stock_adjust";

	public $Notices;
	public $className = "Adjustment_Controller";
	...
	{
		$this->Logic = new WCWH_Adjustment_Class( $this->db_wpdb );
		...
		try
        {
        	...
        	switch ( $action )
        	{
				...
				case "update":
					...
					if( ! $header['apply_all'] ) unset( $header['plus_sign'] );
					if( ! $detail && $header['apply_all'] )
					{
						$header['doc_type'] = 'stock_adjust';
						$strg_id = apply_filters( 'wcwh_get_system_storage', 0, $header, $detail );

						$stocks = $this->Logic->get_available_stock_in_2( $this->warehouse['code'], $strg_id );
						if( $stocks )
						{
							$detail = [];
							foreach( $stocks as $i => $stock )
							{
								$detail[] = [
									...
									'item_id' => '',
									'bqty' => $stock['leftover_qty'],
									'plus_sign' => ( $header['plus_sign'] )? $header['plus_sign'] : '-',
								];
							}
						}
					}
					...
					{	
						foreach( $detail as $i => $item )
						{
							...
							if( $item['plus_sign'] == "+" && $item['bqty'] > 0 && ( empty( $item['uprice'] ) || empty( $item['total_amount'] ) ) )
							{	
								$header['doc_type'] = 'stock_adjust';
								$strg_id = apply_filters( 'wcwh_get_system_storage', 0, $header, $detail );
								if( $strg_id )
								{
									$transact_item = apply_filters( 'warehouse_get_inventory_transaction_item_weighted_price', $item['product_id'], $this->warehouse['code'], $strg_id );

									if( $transact_item )
									{
										if( $transact_item['product_id'] == $item['product_id'] )
											if( ! $detail[$i]['uprice'] ) $detail[$i]['uprice'] = $transact_item['bal_price'];
										else
											if( ! $detail[$i]['uprice'] ) $detail[$i]['uprice'] = $transact_item['converse_uprice'];
									}
								}
							}
						}
	...
	public function im_ex_default_column( $params = array() )
	{
		...
		$default_column['title'] = [ 'Item Code', 'Item Serial' ,'Item Name', 'UOM', 'Inconsistent', 'Qty', 'Unit', 'Price', 'Total Price' ];

		$default_column['default'] = [ 
			'code', 'serial', 'item', 'uom', 'inconsistent', 'bqty', 'bunit', 'unit_price', 'total_price', 
		];
	...
	public function import_data_handler( $datas, $args = array() )
	{
		...
		foreach( $datas as $i => $data )
		{
			...
			$row = [
				...
				'plus_sign' => ( $data['bqty'] >= 0 )? '+' : '-',
			];
			if( empty( $data['bqty'] ) ) $row['plus_sign'] = ( $data['total_price'] >= 0 )? '+' : '-';
			if( ! empty( $data['unit_price'] ) ) $row['uprice'] = $data['unit_price'];
			else if( empty( $data['unit_price'] ) && ! empty( $data['total_price'] ) ) $row['uprice'] = abs( $data['total_price'] ) / $data['bqty'];

			if( empty( $data['bqty'] ) ) $row['uprice'] = 0; 
		...
		switch( strtolower( $type ) )
		{
			...
			?>
				<button class="btn btn-sm btn-primary toggle-modal" data-action="save" data-tpl="<?php echo $this->tplName['new'] ?>" 
					data-title="<?php echo $actions['save'] ?> Adjustment" data-modal="wcwhModalForm" 
					data-actions="close|submit" 
					title="Add <?php echo $actions['save'] ?> Adjustment"
				>
					<?php echo $actions['save'] ?> Adjustment
	...
	public function gen_form( $ids = array() )
	{
		...
		do_action( 'wcwh_get_template', 'form/adjustment-form.php', $args );
	}
	...
	public function view_form( $id = 0, $templating = true, $isView = false )
	{
		...
		if( $id )
		{
			...
			{	
				...
				if( $datas['status'] > 0 )
					$datas['details'] = $this->Logic->get_detail( [ 'doc_id' => $id ], [], false, [ 'uom'=>1, 'usage'=>1, 'transact'=>1, 'meta'=>['plus_sign'] ] );
				else
		        ...
		        if( $datas['details'] )
		        {
		        	foreach( $datas['details'] as $i => $item )
		        	{
		        		...
		        		if( $isView )
		        		{
			        		...
			        		if( ! $datas['details'][$i]['total_amount'] && $item['uprice'] )
			        			$datas['details'][$i]['total_amount'] = round_to( $item['bqty'] * $item['uprice'], 2 );

			        		$datas['details'][$i]['avg_total'] = round_to( $item['weighted_total'], 2 );
			        		$datas['details'][$i]['avg_price'] = round_to( $item['weighted_total'] / $item['bqty'], 5 );
		        		}
		        	}
		        }
		...
		if( $templating )
		{
			do_action( 'wcwh_templating', 'form/adjustment-form.php', $this->tplName['new'], $args );
		}
		else
		{
			do_action( 'wcwh_get_template', 'form/adjustment-form.php', $args );
	...
	public function view_listing( $filters = array(), $order = array() )
	{
		...
		>
		<?php
			include_once( WCWH_DIR . "/includes/listing/adjustmentListing.php" ); 
			$Inst = new WCWH_Adjustment_Listing();

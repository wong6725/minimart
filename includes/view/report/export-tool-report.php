...
?>
    <div class='form-rows-group'>
        ...
        <div class="form-row">
            <div class="col form-group">
            <?php 
                $options = [ 'summary'=>'Credit Summary', 'details'=>"Credit Details" ];
                
                wcwh_form_field( 'export_type', 
                    [ 'id'=>'export_type', 'type'=>'select', 'label'=>'Export Type', 'required'=>true, 'attrs'=>[], 
                        'class'=>['select2Strict'],
                        'options'=> $options
                    ], 
                    ( $args['def_type'] )? $args['def_type'] : 'summary', $view 
                ); 
            ...
            <?php 
                ...
                if( $args['def_type'] == 'summary' )
                {
                    wcwh_form_field( 'acc_type', 
                        [ 'id'=>'acc_type', 'type'=>'select', 'label'=>'Acc Type', 'required'=>false, 'attrs'=>[], 'class'=>['select2'],
                            'options'=> $options
                        ], 
        ...
        <div class="form-row">
            <div class="col form-group">
            <?php 
                $grps = $args['setting']['wh_tool_request']['used_item_group'];
                $def = $args['setting']['wh_tool_rpt']['def_item_group'];

                $filter = [ 'id'=>$grps ];
                if( $filters['seller'] ) $filter['seller'] = $filters['seller'];
                $options = options_data( apply_filters( 'wcwh_get_item_group', $filter, [], false, [] ), 'id', [ 'code', 'name' ], '' );
                
                wcwh_form_field( 'item_group', 
                    [ 'id'=>'acc_type', 'type'=>'select', 'label'=>'Item Group', 'required'=>false, 'attrs'=>[], 'class'=>['select2Strict'],
                        'options'=> $options
                    ], 
                    isset( $filters['item_group'] )? $filters['item_group'] : $def, $view 
                );
            ?>
            </div>
            <div class="col form-group">

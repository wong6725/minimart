...
?>
    <div class='form-rows-group'>
        ...
        <div class="form-row">
            <div class="col form-group">
            <?php 
                $filter = [ 'limit_type'=>$args['limit_type'] ];
                if( $filters['seller'] ) $filter['seller'] = $filters['seller'];
                $ag = [ 'usage'=>0, 'meta'=>['limit_type'] ];
                $options = options_data( apply_filters( 'wcwh_get_promo_header', $filter, [], false, $ag ), 'id', [ 'docno', 'title', 'from_date', 'to_date' ], '' );
                
                wcwh_form_field( 'promotion[]', 
                    [ 'id'=>'promotion', 'type'=>'select', 'label'=>'By Promotion', 'required'=>false, 'attrs'=>[], 
                        'class'=>['select2','modalSelect'],
                        'options'=> $options, 'multiple'=>1
                    ], 
                    $filters['promotion'], $view 
                ); 
            ?>
            </div>
            <div class="col form-group">

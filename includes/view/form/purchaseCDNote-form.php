...
<?php endif; ?>
    ...
    <div class="header-container">
        ...
        <div class="form-row">
            ...
            <?php 
                wcwh_form_field( $prefixName.'[docno]', 
                    [ 'id'=>'', 'label'=>'Document No.', 'required'=>false, 'attrs'=>[], 'description'=>'System Auto Generate' ], 
            ...
            <?php 
                if( $datas['ref_doc_id']  && $datas['ref_doc'] )
                {
                    wcwh_form_field( $prefixName.'[ref_doc]', 
                        [ 'id'=>'', 'type'=>'text', 'label'=>'Reference Doc No.', 'required'=>false, 'attrs'=>[] ], 
                        $datas['ref_doc'], true 
                    );  
                    wcwh_form_field( $prefixName.'[ref_doc]', 
                ...
                {
                    wcwh_form_field( $prefixName.'[ref_doc]', 
                        [ 'id'=>'', 'label'=>'Reference Doc No.', 'required'=>false, 'attrs'=>[],'class'=>[] ], 
                        $datas['ref_doc'], $view 
                    );  
                }
        ...
        <div class="form-row">
            <div class="col form-group">
                <?php 
                    $note_actions = ['2'=>'(-) Issue "Debit Note" to seller [Decrease in Invoice Value]', '1'=>'(+) Issue "Credit Note" to seller [Increase in Invoice Value]'];
                    wcwh_form_field( $prefixName.'[note_action]', 
                        [ 'id'=>'', 'type'=>'select', 'label'=>'Actions', 'required'=>true, 'attrs'=>[], 'class'=>['select2Strict'],
                            'options'=> $note_actions
                        ], 
                        ( $datas['note_action'] )? $datas['note_action'] : $sc, ( $datas['doc_id'] )? 1 : $view 
                    ); 
            ...
            <?php 
                wcwh_form_field( $prefixName.'[inventory_action]', 
                    [ 'id'=>'', 'type'=>'checkbox', 'label'=>'Required Inventory Action', 'required'=>false, 'attrs'=>[] ], 
                    $datas['inventory_action'], $view 
                ); 
            ?>
            </div>
        </div>
        <div class="form-row">
            <div class="col form-group">
            <?php 
                wcwh_form_field( $prefixName.'[note_reason]', 
                    [ 'id'=>'', 'type'=>'textarea', 'label'=>'Reason', 'required'=>true, 'attrs'=>[] ], 
                    $datas['note_reason'], $view 
                );
            ?>
            </div>
        </div>
            ...
            <thead>
                <tr>
                    ...
                    <?php if( $datas['has_tree'] ): ?>
                        <th class="item">Item Child</th>
                    <?php endif; ?>
                    <th class="d_remark">Qty</th>
                    <th class="d_remark">Total Amount</th>
            ...
            <?php         
                if( $datas['details'] )
                {
                    foreach( $datas['details'] as $i => $row )
                    {
                        ...
                        if($datas['option'])
                        {
                            $row['options'] = $datas['option'][$i];
                        }
                     
                        if( $datas['has_tree'] )
                        {
                            $tpl = apply_filters( 'wcwh_get_template_content', 'segment/purchaseCDNoteRef-row.php', $row );
                        }
                        else
                        {
                            $tpl = apply_filters( 'wcwh_get_template_content', 'segment/purchaseCDNote-row.php' );
                        }

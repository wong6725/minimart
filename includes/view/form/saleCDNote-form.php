...
<?php endif; ?>
    ...
    <div class="header-container">
        ...
        <div class="form-row">
            <div class="col form-group">
                <?php 
                    $note_actions = ['2'=>'(+) Issue "Debit Note" to buyer [Increase in Invoice Value]', '1'=>'(-) Issue "Credit Note" to buyer [Decrease in Invoice Value]'];
                    wcwh_form_field( $prefixName.'[note_action]', 
                        [ 'id'=>'', 'type'=>'select', 'label'=>'Actions', 'required'=>true, 'attrs'=>[], 'class'=>['select2Strict'],
                            'options'=> $note_actions, 'description'=>'Credit Note must match with SO Item'
                        ], 
                        ( $datas['note_action'] )? $datas['note_action'] : $sc, $view 
                    ); 
                ?>
            </div>
            ...
            {                
                if( $datas['details'] )
                {
                    foreach( $datas['details'] as $i => $row )
                    {
                        ...
                        $tpl = apply_filters( 'wcwh_get_template_content', 'segment/saleCDNote-row.php' );

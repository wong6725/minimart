    ...
    <div class="detail-container">
        ...
        <div class="actions row">
            ...
            <?php 
                $filter = [ 'status'=>1 ];
                if( $args['seller'] ) $filter['seller'] = $args['seller'];
                $items = apply_filters( 'wcwh_get_item', $filter, [], false, [ 'uom'=>1, 'isUnit'=>1, 'stocks'=>$datas['warehouse_id'], 'needTree'=>1, 'treeOrder'=>['breadcrumb_code','asc'], 'meta'=>['returnable_item'] ] );
                if( $items )
                {
                    $c_items = []; $inventory = [];
            ...
            <div class="col-md-2">
                <?php 
                    if( ! $datas['ref_doc_id'] && $args['setting'][ $args['section'] ]['auto_sales_limit'] > 0 ) 
                        $attr = 'data-limit="'.$args['setting'][ $args['section'] ]['auto_sales_limit'].'"';
                    echo ' <a class="btn btn-sm btn-primary dynamic-action" data-source=".pr-items" data-tpl="'.$args['rowTpl'].'TPL" data-target="#item_row" '.$attr.' >Add +</a>'; 
                ?>
            </div>
        </div>
        <?php endif; ?>
            ...
            <?php
                if( $datas['details'] )
                {
                    foreach( $datas['details'] as $i => $row )
                    {
                        ...
                        $tpl = apply_filters( 'wcwh_get_template_content', 'segment/deliveryOrder-row.php', $ag );

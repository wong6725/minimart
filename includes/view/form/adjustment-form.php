...
// Initialize required array keys to prevent warnings
$args['tplName'] ??= '';
$args['new'] ??= '';
$args['token'] ??= '';
$args['hook'] ??= '';
...
<form id="<?php echo htmlspecialchars($args['tplName']); ?>" 
    class="needValidate <?php echo htmlspecialchars($args['new']); ?> <?php echo htmlspecialchars($view ? 'view' : ''); ?>" 
    action="" method="post" 
...
<?php if( current_user_cans( ['wh_admin_support'] ) ): ?>
    <div class="header-container">
        ...
        <div class="form-row">
            <div class="col form-group">
            <?php 
                wcwh_form_field( $prefixName.'[plus_sign]', 
                    [ 'id'=>'', 'type'=>'select', 'label'=>'Adjust Direction', 'required'=>false, 'attrs'=>[],
                        'options'=> [ ""=>"None", "-"=>"Out -", "+"=>"In +" ]
                    ], 

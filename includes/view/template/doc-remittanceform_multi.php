...
<?php $header = [ 'title'=>'REMITTANCE FORM ']; ?>
<?php do_action( 'wcwh_get_template', 'template/doc-header.php', $header ); ?>
	...
	<?php
	$last_key = end(array_keys($args['detail']));

	foreach ($args['detail'] as $key => $value)
	{
		$detail = $args['detail'][$key];
		...
		{ 
			...
			<div id="header"> 
				<?php do_action( 'wcwh_get_template', 'template/doc-remittanceform-heading.php', $args['heading'][$key] );?>
			</div>

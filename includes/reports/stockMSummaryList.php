...
class WCWH_Movement_Summary_Report extends WCWH_Listing 
{	
	protected $refs;
	...
	{
		$from_month = date( 'Y-m', strtotime( $this->filters['from_month'] ) );
		$to_month = date( 'Y-m', strtotime( $this->filters['to_month'] ) );
		
		$def_fm = date( 'm/1/Y', strtotime( $this->filters['from_month'] ) );
		$def_tm = date( 'm/1/Y', strtotime( $this->filters['to_month'] ) );
	?>
		<div class="row">
			<div class="col-md-3 segment">
				<label class="" for="flag">From Month</label><br>
				<?php
					wcwh_form_field( 'filter[from_month]', 
	                    [ 'id'=>'', 'type'=>'text', 'label'=>'', 'required'=>false, 'class'=>['doc_date', 'picker'],
							'attrs'=>[ 'data-dd-hide-day=1', 'data-dd-format="Y-m"', 'data-dd-default-date="'.$def_fm.'"' ], 'offClass'=>true
	                    ], 
	                    isset( $from_month )? $from_month : '', $view 
	                ); 
				?>
			</div>

			<div class="col-md-3 segment">
				<label class="" for="flag">To Month </label><br>
				<?php
					wcwh_form_field( 'filter[to_month]', 
	                    [ 'id'=>'', 'type'=>'text', 'label'=>'', 'required'=>false, 'class'=>['doc_date', 'picker'],
							'attrs'=>[ 'data-dd-hide-day=1', 'data-dd-format="Y-m"', 'data-dd-default-date="'.$def_tm.'"' ], 'offClass'=>true
	                    ], 
	                    isset( $to_month )? $to_month : '', $view 
	                ); 
				?>
			</div>
	...
	public function print_column_footers( $datas = array(), $all_datas = array() ) 
	{
		...
		foreach ( $columns as $column_key => $column_display_name ) 
		{
			...
			$column_display_name = '';
			if( $column_key == 'month' ) $column_display_name = 'TOTAL:';

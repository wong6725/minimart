...
if ( !class_exists( "WCWH_StockBalance_AJAX" ) )
{
	
class WCWH_StockBalance_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "StockBalance_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'stock_balance_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_StockBalance_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/stock-balance.php" ); 
    }

    public function submission()
    {
		$section_id = 'balance_export';
        $datas = $_REQUEST;
    ...
    {
        $section_id = 'stock_balance_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportBalanceTPL'] = $this->renew_form( $Inst, 'export_form', 'stock_balance' );
        //$outcome['segments']['#printBalanceTPL'] = $this->renew_form( $Inst, 'printing_form', 'stock_balance' );
...
new WCWH_StockBalance_AJAX( $refs );
}
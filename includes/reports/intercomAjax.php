...
if ( !class_exists( "WCWH_Intercom_Rpt_AJAX" ) )
{
	
class WCWH_Intercom_Rpt_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Intercom_Rpt_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'intercom_company_report' => false,
            'intercom_worker_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Intercom_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/intercom.php" ); 
    }

    public function submission()
    {
		$section_id = 'intercom_report_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function intercom_company_report()
    {
        $section_id = 'intercom_company_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportIntercomTPL'] = $this->renew_form( $Inst, 'export_form', 'company' );
        $outcome['segments']['#printIntercomTPL'] = $this->renew_form( $Inst, 'printing_form', 'company' );

        echo json_encode( $outcome );
        die();
    }

    public function intercom_worker_report()
    {
        $section_id = 'intercom_worker_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportIntercomTPL'] = $this->renew_form( $Inst, 'export_form', 'worker' );
        $outcome['segments']['#printIntercomTPL'] = $this->renew_form( $Inst, 'printing_form', 'worker' );
...
new WCWH_Intercom_Rpt_AJAX( $refs );
}
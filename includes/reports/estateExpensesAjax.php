...
if ( !class_exists( "WCWH_EstateExpenses_Rpt_AJAX" ) )
{
	
class WCWH_EstateExpenses_Rpt_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "EstateExpenses_Rpt_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'estate_expenses_detail_report' => false,
            'estate_expenses_category_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_EstateExpenses_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/estateExpenses.php" ); 
    }

    public function submission()
    {
		$section_id = 'estate_expenses_report_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function estate_expenses_detail_report()
    {
        $section_id = 'estate_expenses_detail_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportEstateExpensesTPL'] = $this->renew_form( $Inst, 'export_form', 'detail' );
        $outcome['segments']['#printEstateExpensesTPL'] = $this->renew_form( $Inst, 'printing_form', 'detail' );

        echo json_encode( $outcome );
        die();
    }

    public function estate_expenses_category_report()
    {
        $section_id = 'estate_expenses_category_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportEstateExpensesTPL'] = $this->renew_form( $Inst, 'export_form', 'category' );
        $outcome['segments']['#printEstateExpensesTPL'] = $this->renew_form( $Inst, 'printing_form', 'category' );
...
new WCWH_EstateExpenses_Rpt_AJAX( $refs );
}
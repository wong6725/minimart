...
if ( !class_exists( "WCWH_POSCost_AJAX" ) )
{
    
class WCWH_POSCost_AJAX extends WCWH_AJAX
{
    protected $refs;
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'pos_item_cost_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_POSCost_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/posCost.php" ); 
    }

    public function submission()
    {
        $section_id = 'pos_cost_report_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function pos_item_cost_report()
    {
        $section_id = 'pos_item_cost_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportPOSCostTPL'] = $this->renew_form( $Inst, 'export_form', 'items' );
        //$outcome['segments']['#printPOSCostTPL'] = $this->renew_form( $Inst, 'printing_form', 'items' );

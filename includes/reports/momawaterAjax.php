...
if ( !class_exists( "WCWH_MOMAwater_AJAX" ) )
{
	
class WCWH_MOMAwater_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "MOMAwater_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
			'momawater_summary_report' => false,
            'momawater_detail_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_MOMAwater_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/momawater.php" ); 
    }

    public function submission()
    {
        $section_id = 'momawater_report_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function momawater_summary_report()
    {
        $section_id = 'momawater_summary_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportMOMAwaterTPL'] = $this->renew_form( $Inst, 'export_form', 'summary' );
        $outcome['segments']['#printMOMAwaterTPL'] = $this->renew_form( $Inst, 'printing_form', 'summary' );

        echo json_encode( $outcome );
        die();
    }

    public function momawater_detail_report()
    {
        $section_id = 'momawater_detail_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportMOMAwaterTPL'] = $this->renew_form( $Inst, 'export_form', 'detail' );
        $outcome['segments']['#printMOMAwaterTPL'] = $this->renew_form( $Inst, 'printing_form', 'detail' );

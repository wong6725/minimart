...
class WCWH_CustomerCredit_Report extends WCWH_Listing 
{	
	protected $refs;
	...
	public function get_columns() 
	{
		...
		return array(
			...
			"uid" 			=> "SAP Emp ID",
			"acc_code"		=> "Acc Type",
			"lpurchase_at"	=> "Last Purchase Date",
			"creditable"	=> "Creditable ({$currency})",
			"credit_used"	=> "Used ({$currency})",
		...
		switch( $action )
		{
			...
			default:
				...
				$btn = '<a class="linkAction  btn btn-xs btn-light btn-none-'.$action.'" title="'.$actions[ $action ].'" 
					data-id="'.$id.'" data-action="'.$action.'" data-service="'.$services.'" data-form="edit-customer_credit_report"
	...
	{	
		$actions['view'] = $this->get_action_btn( $item, 'view', [ 'services'=>'customer_credit_report_detail' ] );
		
		return sprintf( '%1$s %2$s', '<strong>'.$item['name'].'</strong>', $this->row_actions( $actions, true ) );  
	}

	public function column_creditable( $item )
	{
		return round_to( $item['creditable'], 2, 1, 1 );
	}

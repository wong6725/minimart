...
class WCWH_TradeIn_Report_List extends WCWH_Listing 
{	
	protected $refs;
	...
	?>
		<div class="row">
			...
			<div class="col-md-6 segment">
				<label class="" for="flag">By Promotion </label><br>
				<?php
					$filters = [ 'limit_type'=>$this->limit_type ];
					if( $this->seller ) $filters['seller'] = $this->seller;
					$args = [ 'usage'=>0, 'meta'=>['limit_type'] ];
					$options = options_data( apply_filters( 'wcwh_get_promo_header', $filters, [], false, $args ), 'id', [ 'docno', 'title', 'from_date', 'to_date' ], '' );
					
                	$def = array_keys($options);
		            wcwh_form_field( 'filter[promotion][]', 
		                [ 'id'=>'promotion', 'type'=>'select', 'label'=>'', 'required'=>true, 'attrs'=>[], 'class'=>['select2','modalSelect'],
		                    'options'=> $options, 'multiple'=>1
		                ], 
		                !empty( $this->filters['promotion'] )? $this->filters['promotion'] : $def, $view 
		            ); 
				?>
			</div>
			...
			<div class="col-md-6 segment">
				...
				<?php
					$filters = [];
					if( $this->seller ) $filters['seller'] = $this->seller;
					$options = options_data( apply_filters( 'wcwh_get_customer', $filters, [], false, ['usage'=>0] ), 'id', [ 'code', 'uid', 'name', 'status_name' ], '' );

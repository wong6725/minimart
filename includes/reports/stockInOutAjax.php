...
if (!defined('WCWH_DIR')) {
    define('WCWH_DIR', plugin_dir_path(__FILE__));
}

if ( !class_exists( "WCWH_InOut_AJAX" ) )
{
    
class WCWH_InOut_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "StockInOut_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'stock_inout_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_StockInOut_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/stockInOut.php" ); 
    }

    public function submission()
    {
        $section_id = 'movement_export';
        $datas = $_REQUEST;
     ...
     */
    // in/out
    public function stock_inout_report()
    {
        $section_id = 'stock_inout_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportMovementTPL'] = $this->renew_form( $Inst, 'export_form', 'stock_inout' );

...
if ( !class_exists( "WCWH_Estate_Rpt" ) ) 
{
	
class WCWH_Estate_Rpt extends WCWH_CRUD_Controller
{	
	public $Notices;
	public $className = "Estate";
		...
		try
        {
        	...
        	switch ( $action )
        	{
				case "export":
					...
					{
						case 'category':
							$datas['filename'] = 'Estate Office Category ';
						break;
						case 'detail':
							$datas['filename'] = 'Estate Office ';
						break;
					}
	...
	protected function export_data_handler( $params = array() )
	{
		...
		{
			case 'category':
				return $this->get_estate_category_report( $params );
			break;
			case 'detail':
			default:
				return $this->get_estate_detail_report( $params );
			break;
		}
	}
	...
	public function print_handler( $params = array(), $opts = array() )
	{
		...
		switch( $type )
		{
			case 'category':
				...
				$document['header'] = 'Estate Category';
				$document['heading']['company'] = $warehouse['comp_name'];
				$document['heading']['title'] = 'Estate Category';
...
'amount' => [ 'value'=>$vals['amount'], 'class'=>['rightered'], 'num'=>1 ],
			...
			default:
				...
				$document = [];
				$document['config'] = $opts;
				$document['header'] = 'Estate Detail';
				$document['heading']['company'] = $warehouse['comp_name'];
				$document['heading']['title'] = 'Estate Detail';
...
'amount' => [ 'value'=>$vals['amount'], 'class'=>['rightered'], 'num'=>1 ],
	...
	{
		$action_id = 'estate_report_export';
		$args = array(
		...
		{
			case 'category':
				do_action( 'wcwh_templating', 'report/export-et_category-report.php', $this->tplName['export'], $args );
			break;
			case 'detail':
			default:
				do_action( 'wcwh_templating', 'report/export-et_detail-report.php', $this->tplName['export'], $args );
			break;
		}
	}
	...
	public function printing_form( $type = 'summary' )
	{
		...
		{
			case 'category':
				do_action( 'wcwh_templating', 'report/export-et_category-report.php', $this->tplName['print'], $args );
			break;
			case 'detail':
			default:
				do_action( 'wcwh_templating', 'report/export-et_detail-report.php', $this->tplName['print'], $args );
			break;
		}
	}
	 ...
	 *	Estate Detail
	 */
	public function estate_detail_report( $filters = array(), $order = array() )
	{
		$action_id = 'estate_detail_report';
		$token = apply_filters( 'wcwh_generate_token', $action_id );
		?>
		...
		>
		<?php
			include_once( WCWH_DIR."/includes/reports/estateList.php" ); 
			$Inst = new WCWH_Estate_Report();
			...
			if( ! $this->noList )
			{
				$datas = $this->get_estate_detail_report( $filters, $order, [] );
	 ...
	 *	Estate Category
	 */
	public function estate_category_report( $filters = array(), $order = array() )
	{
		$action_id = 'estate_category_report';
		$token = apply_filters( 'wcwh_generate_token', $action_id );
		?>
		<form class="listing-form <?php echo $action_id; ?>-listing-form" action="" method="post" id="edit-<?php echo $action_id; ?>" data-section="<?php echo $action_id; ?>-listing-form" 
			data-token="<?php echo $token; ?>" data-hook="<?php echo $action_id; ?>"
		>
		<?php
			include_once( WCWH_DIR."/includes/reports/estateCatList.php" ); 
			$Inst = new WCWH_Estate_Category_Report();
			$Inst->seller = $this->seller;
			...
			if( ! $this->noList )
			{
				$datas = $this->get_estate_category_report( $filters, $order, [] );
				$datas = ( $datas )? $datas : array();
			}
	 ...
	 *	---------------------------------------------------------------------------------------------------
	 */
	public function get_estate_detail_report( $filters = [], $order = [], $args = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();
	...
	public function get_estate_category_report( $filters = [], $order = [], $args = [] )
	{
		global $wcwh;
		$wpdb = $this->db_wpdb;
		$prefix = $this->get_prefix();

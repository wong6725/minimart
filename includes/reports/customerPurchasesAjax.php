...
if ( !class_exists( "WCWH_CustomerPurchasesRpt_AJAX" ) )
{
    
class WCWH_CustomerPurchasesRpt_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "CustomerPurchasesRpt_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'customer_purchases_summary' => false,
            'customer_purchases_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_CustomerPurchases_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/CustomerPurchases.php" ); 
    }

    public function submission()
    {
        $section_id = 'customer_purchases_report';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function customer_purchases_summary()
    {
        $section_id = 'customer_purchases_summary';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportCustomerPurchasesTPL'] = $this->renew_form( $Inst, 'export_form', 'summary' );
        //$outcome['segments']['#printCustomerPurchasesTPL'] = $this->renew_form( $Inst, 'printing_form', 'purchase' );

        echo json_encode( $outcome );
        die();
    }

    public function customer_purchases_report()
    {
        $section_id = 'customer_purchases_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportCustomerPurchasesTPL'] = $this->renew_form( $Inst, 'export_form', 'purchase' );
        //$outcome['segments']['#printCustomerPurchasesTPL'] = $this->renew_form( $Inst, 'printing_form', 'purchase' );
...
new WCWH_CustomerPurchasesRpt_AJAX( $refs );
}
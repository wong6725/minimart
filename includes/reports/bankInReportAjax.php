...
if ( !class_exists( "WCWH_BankinRpt_AJAX" ) )
{
    
class WCWH_BankinRpt_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "BankinRpt_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'bank_in_report' => false,
            'bank_in_daily_summary' => false,
    ...
    {
        if( ! class_exists( 'WCWH_BankIn_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/bankinReport.php" ); 
    }

    public function submission()
    {
        $section_id = 'bank_in_report_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function bank_in_report()
    {
        $section_id = 'bank_in_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportBankinTPL'] = $this->renew_form( $Inst, 'export_form', 'bankin' );
        $outcome['segments']['#printBankinTPL'] = $this->renew_form( $Inst, 'printing_form', 'bankin' );

        echo json_encode( $outcome );
        die();
    }

    public function bank_in_daily_summary()
    {
        $section_id = 'bank_in_daily_summary';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportBankinTPL'] = $this->renew_form( $Inst, 'export_form', 'bankin_daily' );
        $outcome['segments']['#printBankinTPL'] = $this->renew_form( $Inst, 'printing_form', 'bankin_daily' );

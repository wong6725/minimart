...
if ( !class_exists( "WCWH_CustomerTool_AJAX" ) )
{
    
class WCWH_CustomerTool_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "CustomerTool_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'customer_tool_report' => false,
            'customer_tool_detail_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_CustomerTool_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/customerTool.php" ); 
    }

    public function submission()
    {
        $section_id = 'customer_tool_report';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function customer_tool_report()
    {
        $section_id = 'customer_tool_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        $outcome['segments']['#exportCustomerToolTPL'] = $this->renew_form( $Inst, 'export_form', 'summary' );
        //$outcome['segments']['#printCustomerToolTPL'] = $this->renew_form( $Inst, 'printing_form', 'summary' );
        echo json_encode( $outcome );
        die();
    }

    public function customer_tool_detail_report()
    {
        $section_id = 'customer_tool_detail_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportCustomerToolTPL'] = $this->renew_form( $Inst, 'export_form', 'details' );
        //$outcome['segments']['#printCustomerToolTPL'] = $this->renew_form( $Inst, 'printing_form', 'details' );
...
new WCWH_CustomerTool_AJAX( $refs );
}
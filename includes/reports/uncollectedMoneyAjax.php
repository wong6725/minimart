...
if ( !class_exists( "WCWH_UncollectedMoney_AJAX" ) )
{
    
class WCWH_UncollectedMoney_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "UncollectedMoney_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'uncollected_money_rpt' => false,
            'uncollected_money_rpt_summary' => false,
        ] );
    }
    ...
    {
        $section_id = 'uncollected_money_rpt_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function uncollected_money_rpt()
    {
        $section_id = 'uncollected_money_rpt';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportUncollectedMoneyTPL'] = $this->renew_form( $Inst, 'export_form' );
        $outcome['segments']['#printUncollectedMoneyTPL'] = $this->renew_form( $Inst, 'printing_form');
       
        echo json_encode( $outcome );
        die();
    }

    public function uncollected_money_rpt_summary()
    {
        $section_id = 'uncollected_money_rpt_summary';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportUncollectedMoneyTPL'] = $this->renew_form( $Inst, 'export_form','summary' );
        $outcome['segments']['#printUncollectedMoneyTPL'] = $this->renew_form( $Inst, 'printing_form','summary');
...
new WCWH_UncollectedMoney_AJAX( $refs );
}
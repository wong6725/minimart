...
if ( !class_exists( "WCWH_StockAging_AJAX" ) )
{
    
class WCWH_StockAging_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "StockAging_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'stock_aging_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_StockAging_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/stockAging.php" ); 
    }

    public function submission()
    {
        $section_id = 'stock_aging_report';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function stock_aging_report()
    {
        $section_id = 'stock_aging_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        $outcome['segments']['#exportStockAgingTPL'] = $this->renew_form( $Inst, 'export_form', '' );

...
if ( !class_exists( "WCWH_Discrepancy_AJAX" ) )
{
	
class WCWH_Discrepancy_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Discrepancy_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            ...
            'do_discrepancy_report' => false,
            'gt_discrepancy_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Discrepancy_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/discrepancy.php" ); 
    }

    public function submission()
    {
		$section_id = 'discrepancy_report_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function discrepancy_report()
    {
        $section_id = 'discrepancy_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportDiscrepancyTPL'] = $this->renew_form( $Inst, 'export_form', 'discrepancy' );
        $outcome['segments']['#printDiscrepancyTPL'] = $this->renew_form( $Inst, 'printing_form', 'discrepancy' );

        echo json_encode( $outcome );
        die();
    }

    public function do_discrepancy_report()
    {
        $section_id = 'do_discrepancy_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportDiscrepancyTPL'] = $this->renew_form( $Inst, 'export_form', 'delivery_order' );
        $outcome['segments']['#printDiscrepancyTPL'] = $this->renew_form( $Inst, 'printing_form', 'delivery_order' );

        echo json_encode( $outcome );
        die();
    }

    public function gt_discrepancy_report()
    {
        $section_id = 'gt_discrepancy_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportDiscrepancyTPL'] = $this->renew_form( $Inst, 'export_form', 'good_return' );
        $outcome['segments']['#printDiscrepancyTPL'] = $this->renew_form( $Inst, 'printing_form', 'good_return' );
...
new WCWH_Discrepancy_AJAX( $refs );
}
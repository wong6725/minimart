...
if ( !class_exists( "WCWH_Itemize_Rpt_AJAX" ) )
{
    
class WCWH_Itemize_Rpt_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "Itemize_Rpt_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'itemize_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Itemize_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/itemizeReport.php" ); 
    }

    public function submission()
    {
        $section_id = 'itemize_report';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function itemize_report()
    {   
        $section_id = 'itemize_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        $outcome['segments']['#exportItemizeTPL'] = $this->renew_form( $Inst, 'export_form' );
        $outcome['segments']['#printItemizeTPL'] = $this->renew_form( $Inst, 'printing_form' );
...
new WCWH_Itemize_Rpt_AJAX( $refs );
}
...
if ( !class_exists( "WCWH_Estate_Rpt_AJAX" ) )
{
	
class WCWH_Estate_Rpt_AJAX extends WCWH_AJAX
{
	protected $refs;

	public $Notices;
	public $className = "Estate_Rpt_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'estate_detail_report' => false,
            'estate_category_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_Estate_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/estate.php" ); 
    }

    public function submission()
    {
		$section_id = 'estate_report_export';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function estate_detail_report()
    {
        $section_id = 'estate_detail_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        $outcome['segments']['#exportEstateTPL'] = $this->renew_form( $Inst, 'export_form', 'detail' );
        $outcome['segments']['#printEstateTPL'] = $this->renew_form( $Inst, 'printing_form', 'detail' );

        echo json_encode( $outcome );
        die();
    }

    public function estate_category_report()
    {
        $section_id = 'estate_category_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        $outcome = array();
        ...
        //Listing
        $outcome['segments']['.'.$section_id.'-listing-form'] = $this->renew_listing( $Inst, $form, [], $section_id );
        $outcome['segments']['#exportEstateTPL'] = $this->renew_form( $Inst, 'export_form', 'category' );
        $outcome['segments']['#printEstateTPL'] = $this->renew_form( $Inst, 'printing_form', 'category' );

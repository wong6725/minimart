...
if ( !class_exists( "WCWH_ReceiptCount_AJAX" ) )
{
    
class WCWH_ReceiptCount_AJAX extends WCWH_AJAX
{
    protected $refs;

    public $Notices;
    public $className = "ReceiptCount_AJAX";
    ...
    protected function ajax_events( $ajaxs )
    {
        return array_merge( $ajaxs, [
            'receipt_count_report' => false,
        ] );
    }

    public function includes()
    {
        if( ! class_exists( 'WCWH_ReceiptCount_Rpt' ) ) include_once( WCWH_DIR . "/includes/reports/receiptCount.php" ); 
    }

    public function submission()
    {
        $section_id = 'receipt_count_report';
        $datas = $_REQUEST;
     ...
     *  ---------------------------------------------------------------------------------------------------
     */
    public function receipt_count_report()
    {
        $section_id = 'receipt_count_report';
        $this->Notices->reset_operation_notice();
        $succ = true;
        ...
        $outcome['segments']['#exportReceiptCountTPL'] = $this->renew_form( $Inst, 'export_form', 'summary' );
        $outcome['segments']['#printReceiptCountTPL'] = $this->renew_form( $Inst, 'printing_form', 'summary' );
...
new WCWH_ReceiptCount_AJAX( $refs );
}